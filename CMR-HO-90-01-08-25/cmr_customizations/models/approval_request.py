    ...
    def action_create_purchase_orders(self):
        ...
        for line in self.product_line_ids:
            ...
            if line.unit_price:
                ...
                po_line_vals['Class'] = line.Class.id
        ...
        for rec in self:
            ...
            if rec.product_id and rec.product_id.product_tag_ids:
                ...
                if not tags:
                    return {
                        'warning': {
                            'title': 'Missing Product Tag',
                            'message': 'Please select a tag for this product.'
                        }
                    }

                if tags.filtered(lambda tag: tag.name.capital() == 'Readymade'):
                    if 1050 < rec.pi_rsp_price < 1200:
                        return {
                            'warning': {
                                'title': 'Invalid Margin',
                                'message': 'For ReadyMade products, RSP price must not be between 1050 and 1200.'
                            }
                        }
            else:
                raise ValidationError("No product template tag in product")
