        ...
        for move in self:
            ...
            for line in move.picking_id.purchase_id.order_line:
                if line.product_id == move.product_id:
                    move.family = line.family.id
                    move.category = line.category.id
                    move.class_level_id = line.class_level_id.id
                    move.brick = line.brick.id
                else:
                    move.family = False
                    move.category = False
                    move.class_level_id = False
                    move.brick = False
...
class StockMoveLine(models.Model):
    ...
    def _get_zone_id(self):
        ...
        for move_line in self:
            ...
            for line in move_line.picking_id.purchase_id.order_line:
                if line.product_id == move_line.product_id:
                    move_line.family = line.family.id
                    move_line.category = line.category.id
                    move_line.class_level_id = line.class_level_id.id
                    move_line.brick = line.brick.id
                else:
                    move_line.family = False
                    move_line.category = False
                    move_line.class_level_id = False
                    move_line.brick = False
    ...
    def compute_cost_price_per_unit(self):
        for rec in self:
            if rec.final_cost and rec.quantity:
                ...
                if len(tax_ids) > 1:
                    for tax in tax_ids:
                        if tax.min_amount <= temp_nhcl_cost_price <= tax.max_amount:
                            rec.nhcl_cost_price = temp_nhcl_cost_price * (tax.amount / 100.0) + temp_nhcl_cost_price
                            break
                else:
                    rec.nhcl_cost_price = temp_nhcl_cost_price * (
                                tax_ids.amount / 100.0) + temp_nhcl_cost_price if tax_ids else temp_nhcl_cost_price
            else:
