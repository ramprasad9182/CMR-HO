...
class MrpProduction(models.Model):
    ...
    def _onchange_stock_barcode(self):
        ...
        if self.state not in ['progress']:
            raise ValidationError("You scan only in In-Progress state")
        ...
        lot = self.env['stock.lot'].search([('product_id', '=', product.id), ('name', '=', serial_number),('company_id.nhcl_company_bool','=',True)], limit=1)
        if not lot:
