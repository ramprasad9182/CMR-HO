...
class HoStoreMaster(models.Model):
    ...
    def replicate_product_categories(self, product_categories):
        masters = self.env['nhcl.ho.store.master'].search([('nhcl_store_type','!=','ho'), ('nhcl_active','=',True)])
        for master in masters:
            for product_categ in product_categories:
                stores = product_categ.replication_id.filtered(
                    lambda
                        x: x.date_replication == False and x.nhcl_terminal_ip == master.nhcl_terminal_ip and x.nhcl_api_key == master.nhcl_api_key)
                if not stores:
                ...
                if stores:
                    ...
                    product_categ.send_replication_data()
                    ...
                    if not stores:
                        ...
                        stores = product_template_id.product_replication_id.filtered(
                            lambda
                                x: x.date_replication == False and x.nhcl_terminal_ip == master.nhcl_terminal_ip and x.nhcl_api_key == master.nhcl_api_key)
                    if stores:
                        product_template_id.send_replication_data()
        ...
        for master in masters:
            if product_ids:
                for product_id in product_ids:
                    ...
                    if stores:
                        # product_id.button_fetch_replication_data()
                        product_id.send_replication_data()
            ...
            if hr_employee_ids:
                for hr_employee_id in hr_employee_ids:
                    ...
                    if stores:
                        ...
                        hr_employee_id.send_replication_data()
    ...
    def create(self, vals_list):
        ...
        Model = self.env['nhcl.ho.store.master']

        # Step 1: assign sequence
        for vals in vals_list:
            if not vals.get('nhcl_store_id'):
                vals['nhcl_store_id'] = seq.next_by_code('nhcl.ho.store.master') or 'NEW'

        # Step 2: collect values
        store_names = [v.get('nhcl_store_name') for v in vals_list if v.get('nhcl_store_name')]
        locations = [v.get('nhcl_location_id') for v in vals_list if v.get('nhcl_location_id')]
        ips = [v.get('nhcl_terminal_ip') for v in vals_list if v.get('nhcl_terminal_ip')]
        ports = [v.get('nhcl_port_no') for v in vals_list if v.get('nhcl_port_no')]
        api_keys = [v.get('nhcl_api_key') for v in vals_list if v.get('nhcl_api_key')]

        # Step 3: HO check (only once)
        if any(v.get('nhcl_store_type') == 'ho' for v in vals_list):
            if Model.search([('nhcl_store_type', '=', 'ho')], limit=1):
                raise ValidationError("An HO already exists.")

        # Step 4: batch validations (single queries)

        if store_names:
            if Model.search([('nhcl_store_name', 'in', store_names)], limit=1):
                raise ValidationError("Store Name already exists.")

        if locations:
            if Model.search([('nhcl_location_id', 'in', locations)], limit=1):
                raise ValidationError("Location already exists.")

        if ips:
            if Model.search([('nhcl_terminal_ip', 'in', ips)], limit=1):
                raise ValidationError("Terminal IP already exists.")

        if ports:
            if Model.search([('nhcl_port_no', 'in', ports)], limit=1):
                raise ValidationError("Terminal PORT already exists.")

        if api_keys:
            if Model.search([('nhcl_api_key', 'in', api_keys)], limit=1):
    ...
    def replicate_product_categories_to_store(self, product_categories):
        total_records = len(product_categories)
        batch_size = 1000
        for start in range(0, total_records, batch_size):
            end = min(start + batch_size, total_records)
            for i in range(start, end):
                stores = product_categories[i].replication_id.filtered(
                    lambda
                        x: x.date_replication == False and x.nhcl_terminal_ip == self.nhcl_terminal_ip and x.nhcl_api_key == self.nhcl_api_key)
                if not stores:
                    product_categories[i].get_stores_data()
                    stores =  product_categories[i].replication_id.filtered(
                        lambda
                            x: x.date_replication == False and x.nhcl_terminal_ip == self.nhcl_terminal_ip and x.nhcl_api_key == self.nhcl_api_key)
                if stores:
                    product_categories[i].send_replication_data_to_store(stores)
                    if len( product_categories[i].replication_id) == len( product_categories[i].replication_id.filtered(
                            lambda x: x.date_replication == True)):
                        product_categories[i].update_replication = True
    ...
    def call_new_store_master_functions(self, old_store):
        ...
        for model_name in model_names:
            ...
            existing = self.env['master.data'].sudo().search([
