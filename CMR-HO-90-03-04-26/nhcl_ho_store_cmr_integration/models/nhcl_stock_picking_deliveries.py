...
class Picking(models.Model):
    ...
    def get_receipts_data(self):
        ...
        try:
            location_data = session.get(
                f"{base_url}/stock.location/search?domain={[('usage', '=', 'supplier')]}", timeout=10
            ).json()
            dest_data = session.get(
                f"{base_url}/stock.location/search?domain={[('name', '=', self.location_dest_id.name)]}", timeout=10
            ).json()
    ...
    def get_delivery_orders(self):
        ...
        location = session.get(f"{location_url}?domain={[('usage', '=', 'supplier')]}").json()
        partner_suffix = self.partner_id.name.split('-')[-1]
        location_dest = session.get(f"{location_url}?domain={[('name', '=', partner_suffix)]}").json()
