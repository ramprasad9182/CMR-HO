...
class Inventory(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if audit_type == 'ho':
                vals['name'] = seq.next_by_code('stock.inventory') or 'New'
            else:
                vals['name'] = seq.next_by_code('stock.inventory.store') or 'New'
...
class InventoryLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        valid_lines = records.filtered(
            lambda l: l.difference_qty and not l.is_done
        )
        if not valid_lines:
            return records
        # Step 2: process in chunks (important for 5K+)
        BATCH_SIZE = 1000
...
class StoresAuditPlansData(models.Model):
    ...
    @api.model_create_multi
    def create(self, vals_list):
        seq = self.env['ir.sequence'].sudo().with_context(force_company=False)
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == 'New':
                vals['name'] = seq.next_by_code('stores.audit.plans.data') or 'New'
        return super().create(vals_list)
