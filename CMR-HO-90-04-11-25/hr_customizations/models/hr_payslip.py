...
class HrPayslip(models.Model):
    ...
    # --- Computed Fields ---
    month_days = fields.Float(string="Month Days", compute="_compute_month_days", store=True)
    ...
    def _compute_month_days(self):
        """Compute total number of days in the payslip's month."""
        for rec in self:
            if rec.date_from:
                year = rec.date_from.year
                month = rec.date_from.month
                rec.month_days = monthrange(year, month)[1]
            else:
                rec.month_days = 0.0
