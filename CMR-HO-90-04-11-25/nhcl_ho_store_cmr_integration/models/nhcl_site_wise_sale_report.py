...
class NhclPOSSaleReport(models.Model):
    ...
    def get_pos_order_sale_report(self):
        self.nhcl_pos_sale_report_ids.unlink()  # Clear previous entries
        try:
            for store in self.nhcl_store_id:
                ...
                # Initialize the total amount for the store
                total_amount = 0.0
                quantity = 0
                ...
                for data in pos_data_line:
                    ...
                    if from_date <= date_order_local <= to_date:
                        # Add to the total amount for this store
                        total_amount += data.get("price_subtotal", 0.0)
                        quantity += data.get("qty", 0.0)
                # After processing all lines, create or update the report line for this store
                if total_amount > 0.0:
                    # Check if the store already has an entry
                    existing_line = self.nhcl_pos_sale_report_ids.filtered(
                        lambda line: line.nhcl_store_id.id == store.id)

                    if existing_line:
                        # Update the existing entry if it exists (this will be a single line per store)
                        existing_line.write({
                            'nhcl_amount_total': existing_line.nhcl_amount_total + total_amount,
                            'nhcl_quantity': existing_line.nhcl_quantity + quantity
                        })
                    else:
                        # Create a new entry for the store if it doesn't exist yet
    ...
    def get_excel_sheet(self):
        ...
        # Write data headers
        headers = ['Company','Total Amount ', 'Quantity']
        ...
        for row_num, line in enumerate(self.nhcl_pos_sale_report_ids, start=1):
            ...
            worksheet.write(row_num, 2, line.nhcl_quantity)
