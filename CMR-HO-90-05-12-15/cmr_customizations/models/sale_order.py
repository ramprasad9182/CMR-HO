...
class SaleOrder(models.Model):
    ...
    def get_picking_lines(self):
        # Process used combinations efficiently without loading full relational fields
        used_lines = self.env['sale.order.line'].sudo().search_read(
            domain=[
                ('order_id', '!=', self.id),
                ('sale_serial_type', '!=', False),
                ('lot_ids', '!=', False),
            ],
            fields=['lot_ids', 'sale_serial_type']
        )
        ...
        for ul in used_lines:
            if ul['lot_ids']:
                lots = lot_model.browse(ul['lot_ids']).read(['name'])
                sale_serial_type = ul['sale_serial_type']
                for lot in lots:
                    used_combinations.add((lot['name'], sale_serial_type))

        # Clear existing lines without prefetch explosion
        self.order_line.with_context(prefetch_fields=False).unlink()
        ...
        picking = self.env['stock.picking'].sudo().search([
            ('name', '=', self.picking_document.name)
        ], limit=1)

        for line in picking.move_line_ids_without_package:
            lot = line.lot_id
            if not lot or lot.product_qty <= 0:
                continue

            if (lot.name, lot.serial_type) in used_combinations:
                continue

            replicated = lot.product_id.product_replication_list_id.filtered(
                lambda x: x.store_id.id == self.partner_id.id)
            if replicated and not replicated.date_replication:
                raise ValidationError(
                    f"This Article {lot.product_id.display_name} not integrated to store.")

            barcodes = lot.ref and [lot.ref] or []
            branded_barcode_value = ', '.join(set(barcodes)) or lot.product_id.barcode
    ...
    def _onchange_barcode_scanned(self):
        ...
        # ------------------------------------------
        # NEW: Merge helper for lots (with availability check)
        # ------------------------------------------
        def merge_or_create_lot_line(product, lot, qty, sale_serial_type, existing_order_line_cmds, location):
            # compute total stock quantity for this lot at the dest/location
            move_lines = self.env['stock.move.line'].sudo().search([
                ('company_id.nhcl_company_bool', '=', True),
                ('lot_id', '=', lot.id),
                ('location_dest_id.id', '=', location)
            ])
            total_stock_qty = sum(move_lines.mapped('quantity'))

            # existing qty across sale orders (includes this order's current lines)
            existing_qty = global_lot_qty(lot, sale_serial_type)

            # If there is an existing line for the same lot & sale_serial_type, we will attempt to increase it
            existing_line = self.order_line.filtered(
                lambda l:
                l.product_id.id == product.id and
                l.sale_serial_type == sale_serial_type and
                lot.id in l.lot_ids.ids
            )
            # Check availability: after adding qty, total allocated across orders should not exceed stock
            # existing_qty currently includes the existing_line quantity (if present). So check existing_qty + qty <= total_stock_qty
            if existing_qty + qty + sum(existing_line.mapped('product_uom_qty')) > total_stock_qty:
                raise ValidationError(
                    f"Qty for lot {lot.name} exceeds available stock..")
            ...
            if existing_line:
                ...
                lot.is_uploaded = True
                return existing_order_line_cmds  # do not add new line
            ...
            lot.is_uploaded = True
            return [new_line] + existing_order_line_cmds
        ...
        if re.match(gs1_pattern, barcode):
            ...
            if product.tracking == 'serial':
                ...
                if existing_orders:
                    raise ValidationError(f"Serial {scanned_number} already used in: {', '.join(set(existing_orders))}")

                qty = 1

                # Always add new line for serial
                new_line = (0, 0, {
                    'product_id': product.id,
                    'family_id': lot.product_id.categ_id.parent_id.parent_id.parent_id.id,
                    'category_id': lot.product_id.categ_id.parent_id.parent_id.id,
            ...
            # LOT PRODUCT  (MERGE HERE)
                ...
                total_stock_qty = sum(lot_qty.mapped('quantity'))

                if existing_qty + qty > total_stock_qty:
                ...
                # *** MERGE OR CREATE (with availability check inside) ***
                self.order_line = merge_or_create_lot_line(product, lot, qty,
                                                           sale_serial_type,
                                                           existing_order_line_cmds,
                                                           location)
        ...
        # ------------------------------------------
            ...
            if product.tracking not in ('serial', 'lot'):
                raise ValidationError('Product must be tracked.')

            remaining_qty = self.entered_qty or 0
            if remaining_qty <= 0 and product.tracking == 'lot':
            ...
            for quant in lots:
                ...
                if product.tracking == 'serial':
                    ...
                    if lot.rs_price <= 0.0:
                        raise ValidationError(
                            "You are not allowed to add serials not done with landed cost."
                        )
                ...
                # LOT PRODUCT  (MERGE HERE)
                    ...
                    existing_line = self.order_line.filtered(
                        lambda l: l.product_id.id == product.id
                                  and l.sale_serial_type == sale_serial_type
                                  and lot.id in l.lot_ids.ids
                    )
                    ...
                    available_qty = total_stock_qty - existing_qty - existing_line.product_uom_qty
                    if existing_line:
                        # available after counting existing_line: existing_qty already includes existing_line.product_uom_qty
                        # We check if existing_qty + allocate_qty <= total_stock_qty (same as earlier helper)
                        allocate_qty = min(remaining_qty, total_stock_qty - existing_qty - existing_line.product_uom_qty)
                        if allocate_qty <= 0:
                            continue
                        # perform merge (availability already validated)
                        existing_line.product_uom_qty += allocate_qty
                        existing_line.price_unit = lot.cost_price
                        lot.is_uploaded = True
                        remaining_qty -= allocate_qty
                    else:
                        # no existing line: allocate new line after checking availability
                        if available_qty <= 0:
                            continue
            ...
            if not allocated_lines and remaining_qty > 0 and product.tracking == 'lot':
                raise ValidationError(
            ...
            if remaining_qty > 0:
                raise ValidationError(
                    f"Only {self.entered_qty - remaining_qty} qty allocated. Not enough available lots."
                )
        ...
        # ------------------------------------------
            ...
            for lot in lots.lot_id:
                ...
                # Lot product (MERGE HERE)
                    ...
                    # If same lot exists in order, we attempt to merge — availability will be checked in helper
                    if any(
                            lot.name in line.lot_ids.mapped('name')
                            and line.sale_serial_type == sale_serial_type
            ...
            if not selected_lot:
                raise ValidationError(f"No available lot found for {prefix} meeting constraints.")

            if selected_lot.rs_price <= 0.0:
                raise ValidationError("You are not allowed to add serials not done landed cost.")
            ...
            # *** MERGE OR CREATE (with availability check inside) ***
            self.order_line = merge_or_create_lot_line(
    ...
    #             location = self.env.ref('stock.stock_location_stock').id
    #             Quant = self.env['stock.quant']
    #             lots = Quant.search([('quantity', '>', 0),('company_id', '=', self.company_id.id),
    #                 ('location_id', '=', location),('lot_id.name', '=', prefix),('lot_id.type_product', '=', 'un_brand')])
    #             if not lots:
    #                 lots = Quant.search([('quantity', '>', 0),('company_id', '=', self.company_id.id),
    #                     ('location_id', '=', location),('lot_id.ref', '=', prefix),('lot_id.type_product', '=', 'brand')])
    #             if not lots:
    #                 raise ValidationError(f"No lots found for custom barcode {prefix}")
    #             selected_lot = None
    #             qty = 0
