...
class Picking(models.Model):
    ...
    def check_stock_verification_vs_move_lines(self):
        for picking in self:
            ...
            # 🔹 Serial numbers from verification lines
            verification_serials = set(
                picking.stock_verification_ids
                .mapped('stock_serial')  # <-- change field name if different
            )

            # 🔹 Lot names from move lines
            move_line_serials = set(
                picking.move_line_ids_without_package
                .mapped('lot_id.name')
            )

            # Remove empty values just in case
            verification_serials.discard(False)
            move_line_serials.discard(False)

            if verification_serials != move_line_serials:
                missing_in_moves = verification_serials - move_line_serials
                missing_in_verification = move_line_serials - verification_serials

                error_msg = _("Stock verification mismatch:\n")

                if missing_in_moves:
                    error_msg += _(
                        "\n• These serials are not verified but present in detailed lines:\n%s"
                    ) % ", ".join(sorted(missing_in_moves))

                if missing_in_verification:
                    error_msg += _(
                        "\n• These serials are present in move lines but not verified:\n%s"
                    ) % ", ".join(sorted(missing_in_verification))
    ...
    def button_validate(self):
        ...
        for picking in self:
            ...
            if picking.picking_type_id.code == 'incoming' and (picking.stock_type in ['ho_operation','sub_contract','data_import'] or is_subcontract):
                ...
                if move_lines:
                    ...
                    if (not picking.is_confirm and un_move_lines) or (
                        ...
                        if res and picking.purchase_id and picking.is_landed_cost == 'no' and picking.stock_type in ['ho_operation','sub_contract','data_import']:
            ...
            if picking.stock_picking_type == 'return' and picking.company_id.nhcl_company_bool == False:
...
class StockMove(models.Model):
    ...
    def write(self, vals):
        ...
        for move in self:
            if move.picking_id.stock_type in ['ho_operation','sub_contract']:
...
class StockMoveLine(models.Model):
    ...
    def lot_values_update(self):
        for move_line in self:
            if move_line.picking_id.picking_type_code == 'incoming' and move_line.picking_id.stock_type in ['ho_operation','sub_contract','data_import']:
                move_line.lot_id.write({
                    'type_product': move_line.type_product,
...
class StockBackorderConfirmation(models.TransientModel):
    ...
    def process(self):
        for pick_id in self.pick_ids:
            if pick_id.picking_type_id.code == 'incoming' and pick_id.stock_type in ['ho_operation','data_import'] and pick_id.is_confirm == False:
...
class AdjustmentLines(models.Model):
    ...
    def compute_cost_price_per_unit(self):
        for rec in self:
            if rec.final_cost and rec.quantity and rec.cost_id.picking_ids.stock_type in ['ho_operation','data_import']:
