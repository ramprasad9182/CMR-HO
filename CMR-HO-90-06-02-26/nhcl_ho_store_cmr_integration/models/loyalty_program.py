...
class ProductCategory(models.Model):
    ...
    def get_store_category_id(self, ho_ip, ho_port, headers_source):
        for rec in self:
            ...
            discount_product_category_domain = [('display_name', '=', rec.display_name)]
    ...
    def get_store_category_ids(self, ho_ip, ho_port, headers_source):
        product_category_lst = []
        for rec in self:
            discount_product_category_url = f"http://{ho_ip}:{ho_port}/api/product.category/search"
            discount_product_category_domain = [('nhcl_id', '=', rec.nhcl_id)]
            discount_prod_categ_store_url = f"{discount_product_category_url}?domain={discount_product_category_domain}"
            discount_dest_category_data = requests.get(discount_prod_categ_store_url,
                                                       headers=headers_source).json()
            if not discount_dest_category_data.get("data"):
            ...
            discount_category_name = discount_dest_category_data.get("data")[0]
            discount_product_category_id = discount_category_name["id"]
            product_category_lst.append(discount_product_category_id)
...
class ProductAttributeValue(models.Model):
    ...
    def get_store_attribute_vals(self, ho_ip, ho_port, headers_source):
        ...
        for rec in self:
            ...
            category_domain = [('nhcl_id', '=', rec.nhcl_id)]
...
class ProductTag(models.Model):
    ...
    def get_store_product_tag_ids(self, ho_ip, ho_port, headers_source):
        product_tag_lst = []
        for rec in self:
            product_tag_url = f"http://{ho_ip}:{ho_port}/api/product.category/search"
            product_tag_domain = [('name', '=', rec.name)]
            product_tag_store_url = f"{product_tag_url}?domain={product_tag_domain}"
            product_tag_data = requests.get(product_tag_store_url,
                                                       headers=headers_source).json()
            if not product_tag_data.get("data"):
                continue
            product_tag_name = product_tag_data.get("data")[0]
            product_tag_id = product_tag_name["id"]
            product_tag_lst.append(product_tag_id)
        return product_tag_lst
...
class LoyaltyProgram(models.Model):
    ...
    def default_get(self, fields_list):
        res = super(LoyaltyProgram, self).default_get(fields_list)
        if self.env.context.get('menu_type') == 'gift_ewallet':
            res['pos_report_print_id'] = self.env.ref('loyalty.report_gift_card').id
        return res
            ...
            for j in i.nhcl_store_data_id:
                if j.model_id.name == 'Loyalty Program' and j.nhcl_line_data == True:
                    if self.program_type in ['gift_card','ewallet']:
                        ...
                        if i.nhcl_store_name.company_id.id not in self.nhcl_company_ids.ids:
                            self.loyalty_program_id.filtered(lambda x: x.store_id == i.nhcl_store_name).unlink()
                        if i.nhcl_store_name.id in existing_store_ids:
                            continue
    ...
    def get_serial_number_stores(self):
        if self.program_type in ['gift_card','ewallet']:
            ...
            if self.rule_ids[0].type_filter in ['cart','grc','filter','serial']:
                if self.rule_ids.serial_ids:
                    self.nhcl_company_ids = self.rule_ids.serial_ids.mapped('company_id')
                else:
            ...
            if not line.date_replication and line.is_replicate == True:
                ...
                for data in self.rule_ids:
                    ...
                    if data.product_tag_id:
                        search_store_url_product_tag_data = f"http://{ho_ip}:{ho_port}/api/product.tag/search"
                        product_tag_domain = [('nhcl_id', '=', data.product_tag_id.nhcl_id)]
                        product_tag_store_url = f"{search_store_url_product_tag_data}?domain={product_tag_domain}"
                        product_tag_data = requests.get(product_tag_store_url, headers=headers_source).json()
                        if not product_tag_data.get("data"):
                            continue
                        tag_name = product_tag_data.get("data")[0]
                        tag_id = tag_name["id"]
                    ...
                    branch_rule_ids_lst.append((0, 0, {
                        ...
                        'product_tag_id': data.product_tag_id.id,
                        'serial_nos': data.serial_nos,
                ...
                for reward in self.reward_ids:
                    ...
                    rewards_lst.append((0, 0, {
                            ...
                            'is_custom_description': reward.is_custom_description,
                            ...
                            'reward_price' : reward.reward_price if reward.reward_price else 0.0,
                            'buy_product_value' : reward.buy_product_value if reward.buy_product_value else 0,
                            'buy_with_reward_price': reward.buy_with_reward_price if reward.buy_with_reward_price else '',
                ...
                try:
                    ...
                    try:
                        ...
                        # }
                        if self.program_type not in ['gift_card','ewallet']:
                            self.with_company(line.store_id.company_id.id).copy(
                                {'company_id': line.store_id.company_id.id,'nhcl_id': self.nhcl_id,
                                 'rule_ids':branch_rule_ids_lst,'reward_ids':branch_rewards_lst,
                                 'update_replication':False,'coupon_ids':coupon_lst})
                            # self.env['loyalty.program'].:ate(branch_promotion_vals)
    ...
    def update_loyalty_program_data(self):
        for line in self.loyalty_program_id:
            ...
            try:
                ...
                rule_value_data = program_data[0].get('rule_ids')
                rule_length = len(rule_value_data)
                # Retrieve reward values based on program_id
                response2 = requests.get(store_url2, headers=headers_source, json=payload)
                response2.raise_for_status()
                data2 = response2.json()
                reward_value_data = program_data[0].get('reward_ids')
                reward_length = 0
                if reward_value_data:
                    reward_length = len(reward_value_data)
                ...
                rule_line = []
                for index, data in enumerate(self.rule_ids):
                    product_category_lst = []
                    ref_product_lst = []
                    product_tag_lst = []
                    category_1_lst = []
                    ...
                    if data.category_6_ids:
                        category_6_lst = data.category_6_ids.get_store_attribute_vals(ho_ip, ho_port,
                                                                                      headers_source)
                    if data.category_7_ids:
                        category_7_lst = data.category_7_ids.get_store_attribute_vals(ho_ip, ho_port,
                                                                                      headers_source)
                    if data.category_8_ids:
                        category_8_lst = data.category_8_ids.get_store_attribute_vals(ho_ip, ho_port,
                                                                                      headers_source)
                    if data.description_1_ids:
                        description_1_lst = data.description_1_ids.get_store_product_aging_vals(ho_ip, ho_port,
                                                                                                headers_source)
                    ...
                    if data.description_6_ids:
                        description_6_lst = data.description_6_ids.get_store_attribute_vals(ho_ip, ho_port,
                                                                                            headers_source)
                    if data.description_7_ids:
                        description_6_lst = data.description_7_ids.get_store_attribute_vals(ho_ip, ho_port,
                                                                                            headers_source)
                    if data.description_8_ids:
                        description_6_lst = data.description_8_ids.get_store_attribute_vals(ho_ip, ho_port,
                                                                                            headers_source)
                    if data.product_category_ids:
                        product_category_lst = data.product_category_ids.get_store_category_ids(ho_ip, ho_port,
                                                                                                headers_source)

                    if data.ref_product_ids:
                        ref_product_lst = data.ref_product_ids.get_store_products(ho_ip, ho_port, headers_source)
                    if data.product_tag_id:
                        product_tag_lst = data.product_tag_id.get_store_product_tag_ids(ho_ip, ho_port, headers_source)
                    ...
                    if index < rule_length:
                        ...
                        rule_line.append([1, rule_value_data[index]['id'], {
                            'minimum_qty': data.minimum_qty,
                            'minimum_amount': data.minimum_amount,
                            'type_filter': data.type_filter,
                            ...
                            'reward_point_amount': data.reward_point_amount,
                            'reward_point_mode': data.reward_point_mode,
                            'product_category_ids': product_category_lst if product_category_lst else None,
                            'ref_product_ids': ref_product_lst,
                            'product_tag_id': tag_id,
                            'category_1_ids': category_1_lst if category_1_lst else None,
                            'category_2_ids': category_2_lst if category_2_lst else None,
                            'category_3_ids': category_3_lst if category_3_lst else None,
                            'category_4_ids': category_4_lst if category_4_lst else None,
                            'category_5_ids': category_5_lst if category_5_lst else None,
                            'category_6_ids': category_6_lst if category_6_lst else None,
                            'category_7_ids': category_7_lst if category_7_lst else None,
                            'category_8_ids': category_8_lst if category_8_lst else None,
                            'description_1_ids': description_1_lst if description_1_lst else None,
                            'description_2_ids': description_2_lst if description_2_lst else None,
                            'description_3_ids': description_3_lst if description_3_lst else None,
                            'description_4_ids': description_4_lst if description_4_lst else None,
                            'description_5_ids': description_5_lst if description_5_lst else None,
                            'description_6_ids': description_6_lst if description_6_lst else None,
                            'description_7_ids': description_7_lst if description_7_lst else None,
                            'description_8_ids': description_8_lst if description_8_lst else None,
                            'day_ageing_slab': data.day_ageing_slab if data.day_ageing_slab else None,
                            'serial_nos': data.serial_nos if data.serial_nos else None,
                        }])
                    else:
                        ...
                        rule_line.append([0, 0, {
                            ...
                            'product_category_ids': product_category_lst if product_category_lst else None,
                            'ref_product_ids': ref_product_lst if ref_product_lst else None,
                            'product_tag_id': data.product_tag_id.id,
                            'category_1_ids': category_1_lst if category_1_lst else None,
                            'category_2_ids': category_2_lst if category_2_lst else None,
                            'category_3_ids': category_3_lst if category_3_lst else None,
                ...
                # Rewards
                reward_line = []
                reward_create_payloads = []
                for rew_index, reward in enumerate(self.reward_ids):
                    discount_line_product_id = False
                    if reward.discount_line_product_id:
                        discount_line_product_id = reward.discount_line_product_id.get_store_products(ho_ip, ho_port,
                                                                                                      headers_source)
                    if rew_index < reward_length:
                        # If the value exists, use the existing ID and update the values
                        reward_line.append([1, reward_value_data[rew_index]['id'], {
                            'reward_type': reward.reward_type,
                            'discount': reward.discount,
                            'discount_applicability': reward.discount_applicability,
                            'discount_max_amount': reward.discount_max_amount,
                            'required_points': reward.required_points,
                            'discount_line_product_id': discount_line_product_id[
                                0] if discount_line_product_id else False,
                            'description': reward.description,
                            'buy_with_reward_price': reward.buy_with_reward_price,
                            'buy_product_value': reward.buy_product_value,
                ...
                print('reward_create_payloads', reward_create_payloads)
                ...
                store_reward_url_data = f"http://{ho_ip}:{ho_port}/api/loyalty.reward/create"
                for payload in reward_create_payloads:
                    create_resp = requests.post(store_reward_url_data,
                                                headers=headers_source, json=payload)
