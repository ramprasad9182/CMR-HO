    ...
    def action_import_barcodes(self):
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            # ===================== EAN-13 BARCODE =====================
                ...
                # First find one lot matching EAN-13 reference
                ...
                # Now fetch ALL lots for this product to allow qty splitting
                quant_domain = [
            ...
            if not available_quants:
                skipped_lines.append((barcode, "No stock available for this barcode"))
                continue
            ...
            if product.tracking == 'serial':
                if qty > len(available_quants):
                    skipped_lines.append(
                        (barcode, f"Requested qty {qty} exceeds available serial count {len(available_quants)}"))
                    continue
                ...
                for q in available_quants:
                    ...
                    # If same order already used same lot → block
                    same_order_line = existing_lines.filtered(
                        lambda l: l.order_id.id == self.sale_order_id.id and l.sale_serial_type == sale_serial_type)
                    if same_order_line:
                        skipped_lines.append((barcode, f"Serial {lot.name} already used in same order"))
                        created_count = 0
                        break

                    # If GS1: Block if used in other order
                    other_orders = existing_lines.filtered(lambda l: l.order_id.id != self.sale_order_id.id)
                    if other_orders and 'R' in barcode:
                        skipped_lines.append((barcode, f"Serial {lot.name} already used in another order"))
                        created_count = 0
                        break
            ...
            # ===================== LOT TRACKING (SPLIT QUANTITIES) =====================
                ...
                for q in available_quants:
                    ...
                    # Calculate how much qty is already used in sales (not validated yet)
                    used_qty = sum(
                    ...
                    actual_available_qty = q.quantity - used_qty
                ...
                if remaining_qty > 0:
                    skipped_lines.append((barcode, f"Requested qty exceeds available stock. Short by {remaining_qty}"))
