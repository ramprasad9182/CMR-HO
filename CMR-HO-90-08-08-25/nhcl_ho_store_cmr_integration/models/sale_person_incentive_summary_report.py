...
class SetuSalesPersonIncentiveSummary(models.TransientModel):
    ...
    def action_check_sale_person_incentive_report(self):
        ...
        try:
            for store in self.nhcl_store:
                ...
                if self.sale_person:
                    employee_domain = json.dumps([('nhcl_id', '=', self.sale_person.nhcl_id)])
                    emp_url = f"http://{ho_ip}:{ho_port}/api/hr.employee/search?domain={employee_domain}"
                    employee_data = self._safe_api_call(emp_url, headers_source)
                    if employee_data:
                        employee_id = employee_data[0].get("id")
                        employee_barcode = employee_data[0].get("barcode")
                    else:
                        _logger.info(
                            f"No employee found for '{self.name}', skipping store '{store.nhcl_store_name.name}'.")
                        continue

                # Fetch POS order lines
                order_line_domain = []
                if employee_id:
                    order_line_domain = [('employ_id', '=', employee_id), ('badge_id', '=', employee_barcode)]
                order_line_url = f"http://{ho_ip}:{ho_port}/api/pos.order.line/search?domain={json.dumps(order_line_domain)}"
                order_lines = self._safe_api_call(order_line_url, headers_source)

                for data in order_lines:
                    date_order_str = data.get("create_date")
                    date_order = self._parse_datetime(date_order_str)
                    date_order_local = date_order.astimezone(local).date()
                    ...
                    store_master = self.env['nhcl.ho.store.master'].search([('id', '=', store.id)], limit=1)
                    if not store_master:
                        _logger.warning(f"Store master not found for {store.nhcl_store_name.name}")
                        continue

                    # Fetch related POS order
                    order_id = data.get("order_id")[0]['id']
                    order_domain = json.dumps([('id', '=', order_id)])
                    pos_order_url = f"http://{ho_ip}:{ho_port}/api/pos.order/search?domain={order_domain}"
                    pos_orders = self._safe_api_call(pos_order_url, headers_source)
                    if not pos_orders:
                        continue

                    pos_order_date_str = pos_orders[0].get("date_order")
                    order_date = self._parse_datetime(pos_order_date_str).astimezone(local).date()
                    ...
                    if structure_id.nhcl_incentive_type == 'target':
                        lines = structure_id.incentive_structure_line_ids.filtered(
                            lambda x: x.calculate_based_on == 'pos_order' and
                    ...
                    if data.get("price_subtotal_incl") <= 0 or not lines:
                        continue

                    employee = self.env['hr.employee'].sudo().search(
                        [('name', '=', data.get('employ_id')[0]['name'])], limit=1)
                    ...
                    for rule in lines:
                        ...
                        # Aging check
                        if rule.aging_id:
                            if not self._check_aging_match(data, ho_ip, ho_port, headers_source, rule.aging_id.name):
                                continue

                        # Day aging check
                        if rule.day_ageing_incentive:
                            if not self._check_day_aging_match(data, ho_ip, ho_port, headers_source, order_date,
                                                               rule.day_ageing_incentive):
                                continue

                        # Create employee incentive
                        if employee.job_id.id == rule.role.id:
                            value = self._calculate_incentive(rule, data.get("price_subtotal_incl"))
                            self._create_incentive_line(employee.id, rule, data, order_date, value)

                        # Create manager incentive
                        if employee.parent_id and employee.parent_id.job_id.id == rule.role.id:
                            value = self._calculate_incentive(rule, data.get("price_subtotal_incl"))
                            self._create_incentive_line(employee.parent_id.id, rule, data, order_date, value)

        except Exception as e:
            _logger.error(f"Incentive processing failed: {e}")

    # ------------------- Helper Methods -------------------

    def _safe_api_call(self, url, headers):
        try:
            res = requests.get(url, headers=headers)
            res.raise_for_status()
            return res.json().get("data", [])
        except requests.exceptions.RequestException as e:
            _logger.error(f"API call failed for URL: {url} | Error: {e}")
            return []
    ...
    def _check_aging_match(self, data, ip, port, headers, aging_name):
        for lot_dict in data.get('pack_lot_ids', []):
            lot_data = self._get_lot_data(lot_dict, ip, port, headers)
            if lot_data:
                lot = self.env['stock.lot'].sudo().search([('name', '=', lot_data['lot_name'])], limit=1)
                if lot and lot.description_1.name == aging_name:
                    return True
        return False

    def _check_day_aging_match(self, data, ip, port, headers, order_date, selected_range):
        range_map = {
            '1': (0, 30), '2': (30, 60), '3': (60, 90), '4': (90, 120),
            '5': (120, 150), '6': (150, 180), '7': (180, 210),
            '8': (210, 240), '9': (240, 270), '10': (270, 360)
        }
        min_day, max_day = range_map.get(selected_range, (None, None))
        ...
        for lot_dict in data.get('pack_lot_ids', []):
            ...
            if lot_data:
                ...
                if min_day is not None and min_day <= day_diff < max_day:
                    return True
        return False

    def _get_lot_data(self, lot_dict, ip, port, headers):
        domain = json.dumps([('id', '=', lot_dict.get("id"))])
        url = f"http://{ip}:{port}/api/pos.pack.operation.lot/search?domain={domain}"
        return self._safe_api_call(url, headers)

    def _calculate_incentive(self, rule, base_value):
        if rule.calculation_method == 'percentage':
            return base_value * (rule.incentive_value / 100)
        return rule.incentive_value

    def _create_incentive_line(self, employee_id, rule, data, order_date, amount):
        self.env['sale.person.incentive.line'].create({
            ...
            'order_reference': data.get("order_id")[0]['name'],
            'product_name': data.get("product_id")[1],
            'incentive_rule_name': "{} - {} - {}[{}] - {} - {} - {}[{} - {}]".format(
                rule.incentive_structure_id.name,
                dict(rule._fields['role'].selection).get(rule.role),
                dict(rule._fields['calculate_based_on'].selection).get(rule.calculate_based_on),
                "POS Orders",
                dict(rule._fields['target_based_on'].selection).get(rule.target_based_on),
                dict(rule._fields['calculation_method'].selection).get(rule.calculation_method),
                rule.incentive_value,
                rule.target_value_min,
                rule.target_value_max
            )
