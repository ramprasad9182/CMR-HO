...
class MrpProduction(models.Model):
    ...
    def _onchange_stock_barcode(self):
        ...
        if self.state not in ['confirmed','progress']:
            raise ValidationError("Scanning is allowed only when the document is in Confirmed or In Progress state.")
        ...
        if gs1_match:
            ...
            lots = self.env['stock.lot'].search([('ref', '=', product_barcode), ('product_qty', '>', 0),('company_id.nhcl_company_bool','=',True)])
            if not lots:
            ...
            lot = self.env['stock.lot'].search([('name', '=', serial_number),('company_id.nhcl_company_bool','=',True), ('product_qty', '>', 0)], limit=1)

            if not lot:
                # Fallback to internal_ref_lot
                lots = self.env['stock.lot'].search([('internal_ref_lot', '=', serial_number), ('product_qty', '>', 0),('company_id.nhcl_company_bool','=',True)])
                if not lots:
        ...
        lot = self.env['stock.lot'].search([('product_id', '=', product.id), ('product_qty', '>', 0),('name', '=', serial_number),('company_id.nhcl_company_bool','=',True)], limit=1)
        if not lot:
        ...
        if product.tracking == 'serial':
            ...
            assigned_serial_names = component_line.lot_ids.mapped('name')
            if serial_number in assigned_serial_names:
            ...
            # Total already assigned for this component
            current_assigned_qty = sum(component_line.move_line_ids.mapped('qty_done'))

            if current_assigned_qty >= required_qty:
                raise UserError(
                    f'Cannot assign more lot quantities than required ({required_qty}).'
                )

            # Check if this lot is already used in a move line
            existing_move_line = component_line.move_line_ids.filtered(
                lambda ml: ml.lot_id == lot
            )

            # Quantity already assigned from this lot
            already_assigned_from_lot = sum(existing_move_line.mapped('qty_done'))

            remaining_lot_qty = lot.product_qty - already_assigned_from_lot
            remaining_required_qty = required_qty - current_assigned_qty

            assign_qty = min(remaining_lot_qty, remaining_required_qty)

            if assign_qty <= 0:
                raise UserError(
                    f'Lot {serial_number} has no remaining quantity to assign.'
                )

            if existing_move_line:
                # Update existing line instead of creating a new one
                existing_move_line[0].qty_done += assign_qty
            else:
                move_line_vals = {
                    'lot_id': lot.id,
                    'company_id': self.env['res.company'].sudo().search(
                        [('nhcl_company_bool', '=', True)], limit=1
                    ).id,
                    'product_uom_id': product.uom_po_id.id,
                    'product_id': product.id,
                    'location_id': self.env.ref('stock.stock_location_stock').id,
