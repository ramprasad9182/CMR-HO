...
class ProductMarginApproval(models.Model):
    ...
    _rec_name = 'name'
    _description = "Product Margin Approval"
    ...
    manager_id = fields.Many2one('res.users', string="Manager", compute='compute_get_manager', store=True)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == 'New':
                vals['name'] = seq.next_by_code('product.margin.approval') or 'New'
            ...
            for i in branch_lot:
                ho_store_id = self.env['nhcl.ho.store.master'].search(
                    [('nhcl_store_type', '!=', 'ho'), ('nhcl_active', '=', True),
                     ('nhcl_store_name', '=', i.company_id.name)
                     ]
                )
                try:
...
class ProductMarginApprovalLine(models.Model):
    ...
    _description = "Product Margin Approval Line"
...
class ProductAging(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('name'):
                names.append(vals['name'].strip().lower())

            if vals.get('from_date'):
                from_dates.append(vals['from_date'])

        # Step 2: check duplicate names (optimized)
        if names:
            existing = self.env['product.aging'].search([
                ('name', 'ilike', names)
            ])
            existing_lower = {n.strip().lower() for n in existing.mapped('name')}

            for vals in vals_list:
                name = vals.get('name')
                if name and name.strip().lower() in existing_lower:
                    raise ValidationError(
                        _("Aging Prefix '%s' already exists (case-insensitive).") % name
                    )

        # Step 3: validate dates BEFORE create
        for vals in vals_list:
            ...
            if from_date:
                if from_date.day != 1:
                    raise ValidationError(_("From Date must be the first day of the month."))
            ...
            if from_date and to_date:
                ...
                if to_date != expected_to_date:
                    raise ValidationError(_("To Date must be exactly 1 year after From Date."))

        # Step 4: create records
        records = super().create(vals_list)

        # Step 5: post-create logic
        for rec in records:
            rec.create_monthly_lines()
...
class ProductMarginApprovalImportWizard(models.TransientModel):
    ...
    _description = "Product Margin Approval Import Wizard"
