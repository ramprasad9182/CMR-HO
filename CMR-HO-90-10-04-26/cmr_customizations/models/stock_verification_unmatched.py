...
class StockVerificationUnmatched(models.Model):
    ...
    def create(self, vals_list):
        ...
        serials = [v.get('serial_number') for v in vals_list if v.get('serial_number')]
        barcodes = [v.get('barcode') for v in vals_list if v.get('barcode')]
        delivery_names = [v.get('ho_delivery_number') for v in vals_list if v.get('ho_delivery_number')]

        # Step 2: batch fetch quants
        quant_serial_map = {}
        if serials:
            quants = Quant.search([
            ...
            for q in quants:
                quant_serial_map.setdefault(q.lot_id.name, []).append(q)

        quant_barcode_map = {}
        if barcodes:
            quants = Quant.search([
                ('lot_id.ref', 'in', barcodes),
                ('company_id.nhcl_company_bool', '=', False),
                ('quantity', '>', 0)
            ])
            for q in quants:
                quant_barcode_map.setdefault(q.lot_id.ref, []).append(q)

        # Step 3: batch fetch pickings
        picking_map = {}
        if delivery_names:
            pickings = Picking.search([
                ('name', 'in', delivery_names),
                ('picking_type_id.code', '=', 'outgoing')
            ])
            picking_map = {p.name: p for p in pickings}
        ...
        for rec, vals in zip(records, vals_list):
            ...
            if rec.serial_number:
                quants = quant_serial_map.get(rec.serial_number, [])
                if quants:
                    rec.company_id = quants[0].lot_id.company_id

            # ---- Barcode logic ----
            elif rec.barcode:
                quants = quant_barcode_map.get(rec.barcode, [])
                if quants:
                    rec.company_id = quants.mapped('company_id')

            # ---- Delivery logic ----
            if rec.ho_delivery_number:
                picking = picking_map.get(rec.ho_delivery_number)
                if picking and picking.batch_id:
                    rec.batch_schedule_date = picking.batch_id.scheduled_date
