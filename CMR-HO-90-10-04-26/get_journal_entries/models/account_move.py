...
class AccountAccount(models.Model):
    ...
    @api.depends('code')
    def _compute_account_group(self):
        accounts = self.filtered(lambda a: a.code and a.company_id)
        (self - accounts).group_id = False

        if not accounts:
            return

        # Group accounts by company to keep original SQL logic intact
        accounts_by_company = defaultdict(list)
        for acc in accounts:
            accounts_by_company[acc.company_id.id].append(acc)

        for company_id, acc_list in accounts_by_company.items():
            codes = [a.code for a in acc_list]
            placeholders = ",".join(["(%s)"] * len(codes))

            query = f"""
                SELECT DISTINCT ON (acc.code)
                       acc.code,
                       ag.id
                  FROM (VALUES {placeholders}) AS acc(code)
             LEFT JOIN account_group ag
                    ON ag.company_id = %s
                   AND ag.code_prefix_start <= LEFT(acc.code, char_length(ag.code_prefix_start))
                   AND ag.code_prefix_end   >= LEFT(acc.code, char_length(ag.code_prefix_end))
              ORDER BY acc.code,
                       char_length(ag.code_prefix_start) DESC,
                       ag.id
            """

            params = codes + [company_id]

            self.env.cr.execute(query, params)
            result_map = dict(self.env.cr.fetchall())

            for acc in acc_list:
                group_id = result_map.get(acc.code)
                acc.group_id = group_id or False
                if group_id:
                    group = self.env['account.group'].browse(group_id)
                    acc.sequence = group.sequence
            #             # if group.sequence and not account.sequence:


    # def _compute_account_group(self):
    #     accounts = self.filtered(lambda a: a.code)
    #
    #     # Accounts without code → no group
    #     (self - accounts).group_id = False
    #
    #     if not accounts:
    #         return
    ...
    #                acc.code,
    #                ag.id
    #           FROM (VALUES {values_placeholder}) AS acc(code)
    #      LEFT JOIN account_group ag
    #             ON ag.company_id = %s
    #            AND acc.code >= ag.code_prefix_start
    #            AND acc.code <= ag.code_prefix_end
    #       ORDER BY acc.code,
    #                char_length(ag.code_prefix_start) DESC,
    #                ag.id
    #     """
    #
    #     # ✅ Use current company (NOT root_id)
    #     params = codes + [self.env.company.id]
    #
    #     self.env.cr.execute(query, params)
    ...
    #         group_id = results.get(account.code)
    #         account.group_id = group_id or False
    #
    #         # Optional: assign sequence safely
    #         if group_id:
    #             group = self.env['account.group'].browse(group_id)
    #             # if group.sequence and not account.sequence:
    #             account.sequence = group.sequence
...
class AccountGroup(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if (
                    (not vals.get('sequence') or vals.get('sequence') == 'New')
                    and vals.get('type')
            ):
                type_code = vals.get('type')
                seq_code = self._sequence_code_map.get(type_code)

                if seq_code:
                    vals['sequence'] = seq.next_by_code(seq_code)
