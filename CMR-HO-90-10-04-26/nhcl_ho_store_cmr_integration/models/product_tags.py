...
class ProductTag(models.Model):
    ...
    product_tag_id = fields.One2many('product.tag.replication', 'product_tag_line_id')
    ...
    def create(self, vals_list):
        self.env.cr.execute("SELECT MAX(nhcl_id) FROM product_tag")
    ...
    def get_stores_data(self):
        replication_data = []
        existing_store_ids = self.product_tag_id.mapped('store_id.id')
        ...
        for i in ho_store_id:
            for j in i.nhcl_store_data_id:
                if j.model_id.name == 'Product Tag' and j.nhcl_line_data == True:
        ...
        self.update({'product_tag_id': replication_data})
    ...
    def send_replication_data(self):
        ...
        for line in self.product_tag_id:
            if line.date_replication == False:
                ...
                search_store_url_data = f"http://{ho_ip}:{ho_port}/api/product.tag/search"
                product_tag_domain = [('nhcl_id', '=', self.nhcl_id)]
                store_url = f"{search_store_url_data}?domain={product_tag_domain}"
                try:
                    ...
                    if tag_data:
                        _logger.info(f" '{self.name}' Already exists as Tag on '{ho_ip}' with partner '{ho_port}'.")
                        logging.info(f" '{self.name}' Already exists as Tag on '{ho_ip}' with partner '{ho_port}'.")
                        line.date_replication = True
                        continue
                    store_url_data = f"http://{ho_ip}:{ho_port}/api/product.tag/create"
                    try:
                        ...
                        if response_json.get("success") == False:
                            _logger.info(
                                f"Failed to create Tag {message} '{ho_ip}' with partner '{ho_port}'. Error: ")
                            logging.error(
                                f"Failed to create Tag  {message} '{ho_ip}' with partner '{ho_port}'. Error:")
                            ...
                            _logger.info(
                                f"Successfully created Tag {self.name} {message} '{ho_ip}' with partner '{ho_port}'.")
                            logging.info(
                                f"Successfully created Tag {self.name} {message} '{ho_ip}' with partner '{ho_port}'.")
                            if line.master_store_id.nhcl_sink == False:
                        ...
                        _logger.info(f"Failed to create Tag '{ho_ip}' with partner '{ho_port}'. Error: {e}")
                        logging.error(f"Failed to create Tag '{ho_ip}' with partner '{ho_port}'. Error: {e}")
                        line.date_replication = False
                        if line.master_store_id.nhcl_sink == False:
                            line.master_store_id.create_cmr_replication_log('product.tag',
                                                                            self.id, 500, 'add', 'failure',
                                                                            e)
                        else:
                            line.master_store_id.create_cmr_old_store_replication_log('product.tag',
                                                                                      self.id, 500, 'add', 'failure',
                                                                                      e)
                except requests.exceptions.RequestException as e:
                    _logger.info(
                        f" '{self.name}' Error checking Tag on '{ho_ip}' with partner '{ho_port}'. Error: {e}")
                    logging.error(
                        f" '{self.name}' Error checking Tag on '{ho_ip}' with partner '{ho_port}'. Error: {e}")



class ProductTagIntegration(models.Model):
    _name = 'product.tag.replication'
    _description = "Product Tag Replication"

    product_tag_line_id = fields.Many2one('product.tag', string="Replication")
