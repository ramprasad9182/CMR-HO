            ...
            if not line.date_replication:
                ...
                try:
                    ...
                    for tag in self.product_tag_ids:
                        search_store_url_tag_data = f"http://{ho_ip}:{ho_port}/api/product.tag/search"
                        tag_domain = [('nhcl_id', '=', tag.nhcl_id)]
                        tag_store_url = f"{search_store_url_tag_data}?domain={tag_domain}"
                        tag_data = requests.get(tag_store_url, headers=headers_source).json()
                        if not tag_data.get("data"):
                            continue
                        tag_name = tag_data.get("data")[0]
                        tags.append(tag_name["id"])
        ...
        for line in stores:
            ...
            try:
                ...
                if not res_json.get("success"):
                    ...
                    if line.master_store_id.nhcl_sink == False:
                        line.master_store_id.create_cmr_replication_log(res_json['object_name'], self.id, 200,
                                                                        'add', 'failure', message)
                    continue
...
class ProductReplication(models.Model):
    ...
    _description = "Product Replication"
