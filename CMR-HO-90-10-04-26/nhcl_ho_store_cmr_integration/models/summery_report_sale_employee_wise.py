...
class SetuSalesPersonIncentiveSummaryEmployeeWise(models.TransientModel):
    _name = 'sales.person.incentive.summary.report.employee.wise'
    ...
    sale_person_summary_incentive_employee_wise_ids = fields.One2many('sales.person.incentive.summary.report.employee.wise.line', 'sale_person_incentive_employee_wise_id')
    ...
    def summery_report_site_wise(self):
        self.sale_person_summary_incentive_employee_wise_ids.unlink()
        try:
    ...
    def _create_incentive_line(self, employee_id, incentive_line, pos_line_data, order_date, incentive_amount):
        summary_model = self.env['sales.person.incentive.summary.report.employee.wise.line']
        existing_summary = summary_model.search([
            ('sale_person_id', '=', employee_id),
            ('sale_person_incentive_employee_wise_id', '=', self.id),
        ], limit=1)
    ...
    def get_excel_sheet(self):
        ...
        # Write data rows
        for row_num, line in enumerate(self.sale_person_summary_incentive_employee_wise_ids, start=1):
...
class SetuSalesPersonIncentiveSummaryLineEmployeeWise(models.TransientModel):
    _name = 'sales.person.incentive.summary.report.employee.wise.line'
    _description = "Sales Person Incentive Summary Report Lines"

    sale_person_incentive_employee_wise_id = fields.Many2one('sales.person.incentive.summary.report.employee.wise', string="Sale Icentive Lines")
