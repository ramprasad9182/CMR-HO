...
class ProductProduct(models.Model):
    ...
    family_categ_id = fields.Many2one('product.category', string="Division", store=True)
    class_categ_id = fields.Many2one('product.category', string="Section", store=True)
    brick_categ_id = fields.Many2one('product.category', string="Department", store=True)
    category_name_id = fields.Many2one('product.category', string='Category', related='categ_id', store=True)
    ...
    def replace_prefix(records, old_prefix='CMR', new_prefix='NHL'):
        for rec in records:
            code = rec.default_code
            if code and code.startswith(old_prefix):
                new_code = new_prefix + code[len(old_prefix):]
                if not rec.search_count([('default_code', '=', new_code)]):
