...
class Picking(models.Model):
    ...
    def get_delivery_orders(self):
        ...
        picking_type_data = session.get(f"{picking_type_url}?domain={[('name', '=', 'Receipts')]}").json()
        ...
        for move in self.move_ids:
            ...
            # Dictionaries to store merged LOT lines
            merged_detail_lines = {}
            merged_actual_lines = {}
            ...
            for line in move.move_line_ids:
                ...
                lot_id = line.lot_id.name if line.lot_id else False
                lot_name = line.lot_id.name if line.lot_id else False

                # -------------------------------------------------------
                # 👉 For LOT/Serial Tracked Products — Merge Quantities
                # -------------------------------------------------------
                if line.product_id.tracking == 'lot' and lot_id:

                    # -------------------------
                    # Merge DETAIL line
                    # -------------------------
                    if lot_id not in merged_detail_lines:
                        merged_detail_lines[lot_id] = {
                            'product_id': product_id,
                            'mr_price': mr_price,
                        ...
                        merged_detail_lines[lot_id]['quantity'] += line.quantity

                    # -------------------------
                    # Merge ACTUAL line
                    # -------------------------
                    if lot_id not in merged_actual_lines:
                        merged_actual_lines[lot_id] = {
                            'stock_product_id': product_id,
                        ...
                        merged_actual_lines[lot_id]['stock_qty'] += line.quantity

                # -------------------------------------------------------
                # 👉 For NON LOT products — append normally
                # -------------------------------------------------------
                else:
                    detail_line = {
                        'product_id': product_id,
            ...
            # -------------------------------------------------------
            # 👉 ADD only merged LOT lines (one per serial)
            # -------------------------------------------------------
            for val in merged_detail_lines.values():
                stock_detail_lines.append((0, 0, val))

            for val in merged_actual_lines.values():
                stock_actual_lines.append((0, 0, val))
