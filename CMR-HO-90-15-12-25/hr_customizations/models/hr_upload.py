...
class Hrupload(models.Model):
    ...
    def _compute_lunch_addition_amount(self):
        """Apply Lunch Addition Amount ONLY IF:
           1. hr.upload.date month & year match addition master month/year
           2. lunch_delay falls inside a slab time range
           Otherwise → amount = 0
        """
        for rec in self:
            ...
            # hr.upload date month & year
            upload_month = str(rec.date.month)
            upload_year = rec.date.year

            # FIND ADDITION MASTER FOR THIS MONTH + YEAR
            addition_master = self.env['shift.addition.master'].search([
                ('shift_id', '=', shift_id),
                ('addition_type', '=', 'lunch'),
                ('month', '=', upload_month),
                ('year', '=', upload_year),
            ...
            if not addition_master:
                continue

            # MATCH SLAB RANGE
            for slab in addition_master.slab_ids:
    ...
    @api.depends('lunch_delay', 'break_delay', 'employee_name')
        ...
        for rec in self:
            ...
            if break_master and break_master.slab_ids:
                ...
                if not matched and break_min > to_minutes(slabs[-1].to_time):
                    rec.break_delay_amount = slabs[-1].amount
    ...
    def _compute_full_day_status(self):
        for rec in self:
            ...
            for session in sessions:
                ...
                # AFTERNOON SESSION
                # ──────────────────────────────
                if session.day_period == 'afternoon':
                    if worked_hours + EPS >= 4.5:
                        rec.afternoon_session = 'Present'
...
# 17-11-2025
    ...
    tax_amount = fields.Float(
        string="Tax Amount",
        required=True,
        help="Enter the tax amount for the selected month and state."
    )
