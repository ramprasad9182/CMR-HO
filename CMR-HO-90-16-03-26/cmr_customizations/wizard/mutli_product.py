...
class MultiPurchaseProductWizard(models.TransientModel):
    _name = 'multi.purchase.product.wizard'
    _description = 'Wizard to select multiple products'
    ...
    def select_attribute_product_ids(self):
        ...
        return {
            'name': 'Select Product variants',
            'type': 'ir.actions.act_window',
            'res_model': 'multi.purchase.product.attribute.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'default_available_product_ids': allowed_product_ids,
...
class MultiPurchaseProductAttributeWizard(models.TransientModel):
    _name = 'multi.purchase.product.attribute.wizard'
    _description = 'Wizard to select multiple Product Variants'

    # available_product_ids = fields.Many2many('product.product', 'product_count', string="Available Products")
    available_product_ids = fields.Many2many(
        'product.product',
        'multi_product_attribute_wizard_products_rel',  # unique relation table name
        'wizard_id',  # column name for this model
        'product_id',  # column name for related model
        string="Available Products"
    )
    ...
    def action_add_products(self):
        for product in self.product_ids:
            duplicates = []
            existing_line = self.purchase_order_id.order_line.filtered(lambda x: x.product_id == product)
            if existing_line:
                duplicates.append(product.display_name)
            # zone = product.categ_id
            # if not zone.parent_id.parent_id.parent_id.zone_id:
            ...
            if duplicates:
                raise UserError(
                    _("The following products are already in the purchase request:\n%s\nPlease update the quantity in the existing lines.")
                    % ("\n".join(duplicates))
                )
            else:
    ...
    def _onchange_product_ids(self):
        if self.product_ids:
            for product_id in self.product_ids:
                existing_line = self.purchase_order_id.order_line.filtered(lambda x: x.product_id.id in product_id.ids)
                if existing_line:
                    return {
                        'warning': {
                            'title': "Duplicate Product",
                            'message': _(
                                "Product %s is already in the purchase request. Please update quantity in the existing line.") % (
                                           product_id.display_name),
                        }
                    }
