...
class StoreReplenishment(models.Model):
    ...
    @api.constrains('store_id', 'product_tmpl_id')
    def _check_unique_company_product(self):
        for rec in self:
            if not rec.store_id or not rec.product_tmpl_id:
                continue
            domain = [
            ...
            if self.search_count(domain):
                raise ValidationError("Replenishment already exists for this Company and Product!")
