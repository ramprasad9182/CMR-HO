...
class Inventory(models.Model):
    ...
    product_ids = fields.Many2many(
        ...
        readonly=True,
        # states={"draft": [("readonly", False)]},
        help="Specify Products to focus your inventory on particular Products.", tracking=True,
    )
    ...
    plan_name = fields.Char('Plan Name', copy=False, tracking=True,)
    audit_type = fields.Selection([
        ('ho', 'HO Audit'),
        ('store', 'Store Audit'),
    ], string="Audit Type", default='ho', required=True)
    creation_status = fields.Boolean("Synced Status")
    nhcl_audit_remarks = fields.Text(string="Remarks")
    # source_ref = fields.Char(string="Source Reference")
    source_ref_id = fields.Many2one('stores.audit.plans.data',string="Source Reference Id")
    category_1_ids = fields.Many2many('product.attribute.value', 'audit_category_1', string='Color', copy=False,
    ...
    @api.depends("name","plan_name")
    def _compute_display_name(self):
        super()._compute_display_name()
        for i in self:
            i.display_name = f"{i.plan_name}"
    ...
    @api.model
    def create(self, vals):
        audit_type = vals.get("audit_type")
        if audit_type == "ho":
            vals["name"] = self.env["ir.sequence"].next_by_code("stock.inventory") or "New"
        else:
            vals["name"] = self.env["ir.sequence"].next_by_code("stock.inventory.store") or "New"
        return super(Inventory, self).create(vals)
                    ...
                    if line and line.product_id.tracking == 'lot':
                        ...
                        if inventory_line.qty_done >= 1:
                            raise ValidationError(
                                f'lot/serial number qty Already Updated to {inventory_line.qty_done}.')
                        else:
                            line.qty_done += 1
                            break
            else:
    ...
    def _get_quantities(self):
        ...
        elif self.category_1_ids:
            lots = self.env['stock.lot'].search([
                ('category_1', 'in', self.category_1_ids.ids)])
            if lots:
    ...
    def _get_store_master_config(self):
        ...
        if self.audit_type != 'store':
            raise ValidationError(
                "Store configuration is only applicable for Store Audit records."
            )

        store = self.env['nhcl.ho.store.master'].search([
            ('nhcl_store_name.company_id', '=', self.company_id.id),
            ('nhcl_active', '=', True)
        ], limit=1)
        ...
        if not store:
            raise ValidationError(
                f"No active Store configuration found for company: {self.company_id.name}"
            )
    ...
    def send_audit_plans_data_from_ho_to_stores(self):
        for rec in self:
            rec.ensure_one()
            if rec.creation_status:
                raise ValidationError("This Audit Plan is Already Integrated.")

            store = rec._get_store_master_config()
            ho_id = self.env['nhcl.ho.store.master'].search(
                [('nhcl_active', '=', True), ('nhcl_store_type', '=', "ho")],limit=1)

            if not store:
                raise ValidationError("Store configuration not found.")
            if not ho_id:
                raise ValidationError("HO configuration not found.")

            if not store.nhcl_terminal_ip or not store.nhcl_port_no:
                raise ValidationError("Store IP or Port is not configured.")
            ...
            try:
                company_domain = str([('name', '=', rec.company_id.name)])
                company_url = f"{base_url}/res.company/search?domain={company_domain}"

                company_response = requests.get(company_url, headers=headers, timeout=20)
                company_response.raise_for_status()
                company_json = company_response.json()

                if not company_json.get("data"):
                    raise ValidationError("Company not found in destination database.")

                dest_company_id = company_json["data"][0]["id"]

                warehouse_domain = str([('name', '=', rec.warehouse_id.name)])
                warehouse_url = f"{base_url}/stock.warehouse/search?domain={warehouse_domain}"

                warehouse_response = requests.get(warehouse_url, headers=headers, timeout=20)
                warehouse_response.raise_for_status()
                warehouse_json = warehouse_response.json()

                if not warehouse_json.get("data"):
                    raise ValidationError("Warehouse not found in destination database.")

                dest_warehouse_id = warehouse_json["data"][0]["id"]

                dest_category_id = False
                if rec.stock_inventory_category:
                    dest_category_domain = [('nhcl_id', '=', rec.stock_inventory_category.nhcl_id)]
                    dest_category_url = f"{base_url}/product.category/search?domain={dest_category_domain}"
                    category_response = requests.get(dest_category_url, headers=headers, timeout=20)
                    category_response.raise_for_status()
                    category_json = category_response.json()
                    # if not category_json.get("data"):
                    #     raise ValidationError("category not found in destination database.")
                    if category_json.get("data"):
                        dest_category_id = category_json["data"][0]["id"]

                dest_product_ids = []
                if rec.product_ids:
                    # product_names = rec.product_ids.mapped('name')
                    # product_domain = urllib.parse.quote(str([('name', 'in', product_names)]))
                    for product in rec.product_ids:
                        ...
                        product_response = requests.get(product_url, headers=headers,timeout=20)
                        product_response.raise_for_status()
                        product_json = product_response.json()

                        # if not product_json.get("data"):
                        #     raise ValidationError(
                        #         f"Product '{product.name}' not found in destination."
                        #     )
                        if product_json.get("data"):
                            dest_product_ids.append(product_json["data"][0]["id"])

                dest_descrip1_ids = []
                if rec.description_1_ids:
                    # product_names = rec.product_ids.mapped('name')
                    # product_domain = urllib.parse.quote(str([('name', 'in', product_names)]))
                    for descript in rec.description_1_ids:
                        descript1_domain = [('name', '=', descript.name)]
                        descript1_url = f"{base_url}/product.aging.line/search?domain={descript1_domain}"

                        descript1_response = requests.get(descript1_url, headers=headers, timeout=20)
                        descript1_response.raise_for_status()
                        descript1_json = descript1_response.json()


                        if descript1_json.get("data"):
                            dest_descrip1_ids.append(descript1_json["data"][0]["id"])

                # dest_categ1_ids = []
                # if rec.category_1_ids:
                #     for categ in rec.category_1_ids:
                #         categ_domain = [('name', '=', categ.name)]
                #         categ_url = f"{base_url}/product.attribute.value/search?domain={categ_domain}"
                #
                #         categ_response = requests.get(categ_url, headers=headers, timeout=20)
                #         categ_response.raise_for_status()
                #         categ_json = categ_response.json()
                #
                #         if categ_json.get("data"):
                #             dest_categ1_ids.append(categ_json["data"][0]["id"])

                dest_location_ids = []
                for location in rec.location_ids:
                    location_domain = str([('name', '=', location.name)])
                    location_url = f"{base_url}/stock.location/search?domain={location_domain}"

                    location_response = requests.get(location_url, headers=headers, timeout=20)
                    location_response.raise_for_status()
                    location_json = location_response.json()

                    if not location_json.get("data"):
                        raise ValidationError(
                            f"Location '{location.name}' not found in destination."
                        )

                    dest_location_ids.append(location_json["data"][0]["id"])


                create_url = f"{base_url}/stock.inventory/create"
                ...
                audit_data = {
                    ...
                    "stock_inventory_category": dest_category_id,
                    "product_ids": dest_product_ids,
                    # "category_1_ids": dest_categ1_ids,
                    "description_1_ids": dest_descrip1_ids,
                    "location_ids": dest_location_ids,
                    "from_date_range": rec.from_date_range.strftime('%Y-%m-%d %H:%M:%S')
                    if rec.from_date_range else False,
                    "to_date_range": rec.to_date_range.strftime('%Y-%m-%d %H:%M:%S')
                    if rec.to_date_range else False,
                    "nhcl_audit_remarks": rec.nhcl_audit_remarks,
                    'type_product': rec.type_product,
                    # 'create_user_name': rec.user_id.name,
                ...
                model_name = response_json.get("object_name", "stock.inventory")

                # Create Replication Log
                self.env['store.audit.plan.replication.log'].create({
                    # 'nhcl_serial_no': ,
                ...
                if success:
                    rec.write({'creation_status': True})
                    _logger.info(f"Successfully integrated Audit Plan {rec.name}")
                    return True

                else:
                    rec.write({'creation_status': False})
                ...
                _logger.error(f"Connection Error while sending Audit Plan: {str(e)}")
                return False
...
class InventoryLine(models.Model):
    ...
    product_uom_id = fields.Many2one(
        "uom.uom", "Product Unit of Measure", required=True,  store=True
    )
    ...
    is_done = fields.Boolean(string="Adjustment Done", default=False)

    @api.model
    def create(self, vals_list):
        records = super(InventoryLine, self).create(vals_list)

        # Take only valid lines
        valid_lines = records.filtered(
            lambda l: l.difference_qty != 0 and not l.is_done)
        print("********",valid_lines)
        if valid_lines:
            with self.env.cr.savepoint():
                wizard = self.env['stock.adjustment.name'].create({
                    'quant_ids': [(6, 0, valid_lines.ids)],
...
class StoresAuditPlansData(models.Model):

    _name = "stores.audit.plans.data"
    _description = "Stores Audit Plans"
    _order = "date desc, id desc"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    ...
    # duration = fields.Float("Duration (Hours)",store=True)
    start_date = fields.Datetime("Start Date", default=fields.date.today())
    end_date = fields.Datetime("End Date")
    date = fields.Datetime(
        "Inventory Date",
    ...
    company_ids = fields.Many2many(
        ...
        "Company",
        index=True,
        required=True, domain="[('nhcl_company_bool', '!=', 'True')]",
    )

    product_ids = fields.Many2many(
        "product.product",
        string="Products", domain="[('type', '=', 'product'), '|', ('company_id', '=', False), ('company_id', 'in', company_ids)]",
        # states={"draft": [("readonly", False)]},
    ...
    plan_creation_in_stores = fields.Boolean(string="Plan Creation in Stores", default=False)
    ...
    nhcl_audit_remarks = fields.Text(string="Remarks")
    category_1_ids = fields.Many2many('product.attribute.value', 'audit_cat_1', string='Color', copy=False,
                                      domain=[('attribute_id.name', '=', 'Color')])
    description_1_ids = fields.Many2many('product.aging.line', string="Product Aging", copy=False)
    type_product = fields.Selection([('brand', 'Brand'), ('un_brand', 'UnBrand'), ('others', 'Others')],
                                    string='Brand Type', copy=False)
    ...
    @api.constrains('plan_name', 'company_ids')
    def _check_duplicate_plan(self):
        for rec in self:
            ...
            for plan in plans:
                ...
                common_companies = rec.company_ids & plan.company_ids

                if common_companies:
                    company_names = ", ".join(common_companies.mapped('name'))
                    raise ValidationError(
                        _("Plan '%s' already exists for company(s): %s")
                        % (rec.plan_name, company_names)
                    )

    @api.model
    def create(self, vals):
        if not vals.get("name") or vals.get("name") == "New":
            sequence = self.env["ir.sequence"].sudo().with_context(force_company=False)
            vals["name"] = sequence.next_by_code("stores.audit.plans.data") or "New"

        return super(StoresAuditPlansData, self).create(vals)


    def action_create_store_audit_plans(self):
        for plan in self:
            if plan.plan_creation_in_stores:
                raise ValidationError("Already plan created in stores.")
            ...
            all_success = True
            for company in plan.company_ids:
                warehouse = self.env['stock.warehouse'].search([
                ...
                locations = self.env['stock.location'].search([
                    ('company_id', '=', company.id),
                    ('usage', '=', 'internal'),('is_subcontracting_location','=', False)
                ])
                if not locations:
                    raise ValidationError("Please create the locations for selected Companies.")
                ...
                vals = {
                    ...
                    'audit_type': 'store',
                    'company_id': company.id,
                    'from_date_range': plan.from_date_range,
                    'to_date_range': plan.to_date_range,
                    'warehouse_id': warehouse.id if warehouse else False,
                    'location_ids': [(6, 0, locations.ids)] if locations else [],
                    'product_ids': [(6, 0, plan.product_ids.ids)] if plan.product_ids else [],
                    # 'category_1_ids': [(6, 0, plan.category_1_ids.ids)] if plan.category_1_ids else [],
                    'description_1_ids': [(6, 0, plan.description_1_ids.ids)] if plan.description_1_ids else [],
                    'stock_inventory_category': plan.stock_inventory_category.id if plan.stock_inventory_category else False,
                    "nhcl_audit_remarks": plan.nhcl_audit_remarks,
                    'type_product': plan.type_product
                }
                print("&&&&&",vals)
                # Multi-company safe creation
                # Inventory.with_company(company).create(vals)
                # record = Inventory.with_company(company).create(vals)
                # record.send_audit_plans_data_from_ho_to_stores()
                with self.env.cr.savepoint():
                    record = Inventory.with_company(company).create(vals)
                    success = record.send_audit_plans_data_from_ho_to_stores()
            ...
            if all_success:
                plan.plan_creation_in_stores = True
            else:
                raise ValidationError(
                    "One or more store integrations failed. Plan not marked as completed.Please check the log."
                )


            # Update boolean after successful creation
            # plan.plan_creation_in_stores = True
