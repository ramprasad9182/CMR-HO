...
class MasterSequence(models.Model):
    ...
    nhcl_company_id = fields.Many2one('res.company', string='Company', copy=False)

    @api.constrains('nhcl_company_id')
    def _check_unique_company(self):
        for rec in self:
            if rec.nhcl_company_id:
                existing = self.search([('nhcl_company_id', '=', rec.nhcl_company_id.id), ('id', '!=', rec.id)],
                                       limit=1)
                if existing:
                    raise UserError(f"Record already exists for this company: {rec.nhcl_company_id.name}")
...
class vehicleTransport(models.Model):
    ...
    _description = "Vehicle Transport"
...
class CmrTransport(models.Model):
    ...
    _description = "Vehicle Transport Line"
