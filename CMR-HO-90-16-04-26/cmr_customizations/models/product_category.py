...
class ProductCategory(models.Model):
    ...
    def create(self, vals_list):
        ...
        # Step 2: check duplicate root categories
        if names:
            ...
            for vals in vals_list:
                if (
                        vals.get('name') in existing_names
                        and not vals.get('parent_id')
                ):
                    raise ValidationError(
                        _("A product category with the name '%s' already exists.") % vals['name']
                    )

        # Step 3: validate HSN codes (batch)
        hsn_codes = [v.get('hsn_code') for v in vals_list if v.get('hsn_code')]
        if hsn_codes:
            existing_hsn = HSN.search([('hsn_code', 'in', hsn_codes)])
            valid_hsn = set(existing_hsn.mapped('hsn_code'))

            for vals in vals_list:
                if vals.get('hsn_code') and vals['hsn_code'] not in valid_hsn:
                    raise ValidationError(
        ...
        # Step 5: post-create validation
        for rec in categories:
            rec._validate_margin_lines()
...
class ProductCategoryMarginLine(models.Model):
    ...
    _description = "Product Category Margin Line"
...
class ProductCategoryMrpLine(models.Model):
    ...
    _description = "Product Category MRP Line"
