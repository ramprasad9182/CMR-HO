...
class StoreIndents(models.Model):
    ...
    _description = "Store Indent Reference"
    ...
    pi_quantity = fields.Float(string="PI Quantity", related="po_line_id.product_qty", store=True)
    allocated_quantity = fields.Float(string="Allocated Quantity")
    order_date = fields.Datetime(string="Order Date", related="po_line_id.date_order", store=True)
    company_id = fields.Many2one("res.company", string="Company", related="po_line_id.order_id.company_id", store=True)


    @api.constrains('allocated_quantity')
    def check_allocated_quantity(self):
        for rec in self:
            if rec.pi_quantity and rec.allocated_quantity and rec.pi_quantity < rec.allocated_quantity:
                raise ValidationError('You cannot give more allocated qty than PI Quantity')
        ...
        # ------------------------------------------
            ...
            if product.tracking == 'serial':
                ...
                if self.entered_qty <= 0:
                    raise ValidationError("Need to enter more than 0.")
            ...
            if lots[0].lot_id.type_product == 'un_brand':
                for lot in lots.lot_id:
                    ...
                    # ---------------- LOT ----------------
                        ...
                        if existing_line:
                            new_total_qty = existing_line.product_uom_qty + self.entered_qty
                        else:
                            new_total_qty = self.entered_qty

                        # VALIDATION BEFORE UPDATE
                        if in_lot_qty < (existing_so_qty + out_lot_qty + new_total_qty):
                            raise ValidationError(f"Lot {lot.name} is not enough qty to sale.")

                        # UPDATE AFTER VALIDATION
                        if existing_line:
                            existing_line.product_uom_qty = new_total_qty
                            existing_line.price_unit = lot.cost_price
                        else:
                            self.order_line = [(0, 0, {
                                'product_id': product.id,
...
class SaleOrderLine(models.Model):
    ...
    def create(self, vals_list):
        lot_ids = set()  # use set to avoid duplicates
        # Step 1: extract all lot_ids (single pass)
        for vals in vals_list:
            commands = vals.get('lot_ids', [])
            for command in commands:
                if command[0] == 6:  # replace
                    lot_ids.update(command[2])
                elif command[0] == 4:  # add existing
                    lot_ids.add(command[1])
        if lot_ids:
            # Step 2: single query → only fetch problematic lots
            bad_lots = self.env['stock.lot'].search([
                ('id', 'in', list(lot_ids)),
                ('is_under_plan', '=', True)
            ])
            if bad_lots:
                # show only few names (avoid huge error)
                names = ', '.join(bad_lots.mapped('name')[:5])
                raise ValidationError(
                    _("These Serial/Lot numbers are under Audit plan: %s") % names
                )
        # Step 3: batch create
    ...
    def check_landed_cost_yes(self):
        for lot in self:
            if lot.lot_ids and (lot.lot_ids.cost_price < 1.0 or lot.lot_ids.rs_price < 1.0) and self.company_id.nhcl_company_bool == True:
