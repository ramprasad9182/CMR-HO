...
class VendorReturn(models.Model):
        ...
        for rec in self:
            ...
            if rec.return_type == 'regular':
                ...
                for grc, line_group in lines_by_grc.items():
                    ...
                    moves = return_picking.move_ids_without_package.filtered(lambda x: x.product_uom_qty == 0.0)
        ...
        if promotional_serials:
            for promo in promotional_serials:
                ...
                lines.append((0, 0, {
                    ...
                    'lot_cp': lot.actual_cp,
                    'lot_mrp': lot.mr_price,
                    'lot_rsp': lot.rs_price,
                    'quantity': promo.quantity,
                    'tracking': lot.product_id.tracking,
                    'promo_cp': promo.vendor_return_disc_price,
                    'nhcl_grc_bill_number': lot.picking_id.stock_bill_reference,
                    'nhcl_grc_bill_date': lot.picking_id.nhcl_bill_date,
                }))
                promo.is_used = True
    ...
    def get_branded_products(self):
        if self.brand_barcode:
            ...
            if lots and lots[0].product_id.tracking == 'lot':
                if self.lot_qty:
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            vals['name'] = seq.next_by_code('cmr.vendor.return') or 'New'
...
class VendorReturnLine(models.Model):
    ...
    nhcl_grc_bill_number = fields.Char(string='GRC Bill Number')
    nhcl_grc_bill_date = fields.Date(string='Bill Date')
