...
class ReprintDynamicLabel(models.TransientModel):
    ...
    def _onchange_label_id(self):
        if self.label_id:
            self.file = self.label_id.upload_file
            self.file_name = self.label_id.name
    ...
    def action_preview_label(self):
        ...
        if not self.serial_number_line_ids:
            raise UserError(_("No serial numbers selected to preview."))

        if not self.file:
            raise UserError(_("Please upload a ZPL/PRN file to preview."))

        # Decode ZPL template
        zpl_template = base64.b64decode(self.file).decode('utf-8')

        # Use first serial line for preview
        line = self.serial_number_line_ids[0]

        # Replace placeholders
        zpl_content = (
            zpl_template
            .replace("{line.nhcl_name}", str(line.name or ''))
            .replace("{product_name}", str(line.product_id.categ_id.name or ''))

            .replace("{description_1}", str(line.lot_id.description_1.name or ''))
            .replace("{description_2}", str(line.lot_id.description_2.name or ''))
            .replace("{description_3}", str(line.lot_id.description_3.name or ''))
            .replace("{description_4}", str(line.lot_id.description_4.name or ''))
            .replace("{description_5}", str(line.lot_id.description_5.name or ''))
            .replace("{description_6}", str(line.lot_id.description_6.name or ''))
            .replace("{description_7}", str(line.lot_id.description_7.name or ''))
            .replace("{description_8}", str(line.lot_id.description_8.name or ''))
            .replace("{description_9}", str(line.lot_id.description_9.name or ''))

            .replace("{line.categ_1}", str(line.lot_id.category_1.name or ''))
            .replace("{line.categ_2}", str(line.lot_id.category_2.name or ''))
            .replace("{line.categ_3}", str(line.lot_id.category_3.name or ''))
            .replace("{line.categ_4}", str(line.lot_id.category_4.name or ''))
            .replace("{line.categ_5}", str(line.lot_id.category_5.name or ''))
            .replace("{line.categ_6}", str(line.lot_id.category_6.name or ''))
            .replace("{line.categ_7}", str(line.lot_id.category_7.name or ''))
            .replace("{line.categ_8}", str(line.lot_id.category_8.name or ''))

            .replace("{int(line.mr_price)}", str(line.lot_id.mr_price or 0))
            .replace("{int(line.rs_price)}", str(line.lot_id.rs_price or 0))
        )

        # --- Convert ZPL → PNG via Labelary ---
        dpi = 12  # 12 dpmm = ~300 dpi
        width = "4"
        height = "6"

        url = f"http://api.labelary.com/v1/printers/{dpi}dpmm/labels/{width}x{height}/0/"
        files = {'file': ('preview.zpl', zpl_content.encode('utf-8'))}

        response = requests.post(url, files=files, headers={"Accept": "image/png"})
        if response.status_code != 200:
            raise UserError(f"Label Preview Error:\n{response.text}")

        # Save image
        self.zpl_preview = base64.b64encode(response.content)

        # Reopen wizard with preview
        return {
