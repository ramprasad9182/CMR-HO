...
class HrEmployee(models.Model):
    ...
    def create(self, vals_list):
        ...
        # Step 1: assign barcode (before create)
        for vals in vals_list:
            if not vals.get('barcode'):
                vals['barcode'] = seq.next_by_code('hr.employee.cmr') or '/'

        # Step 2: create employees in batch
        employees = super().create(vals_list)

        # Step 3: filter employees with DOJ
        employees_with_doj = employees.filtered(lambda e: e.date_of_joining)

        if employees_with_doj:
            # Fetch existing contracts in batch
            existing_contracts = self.env['hr.contract'].search([
                ('employee_id', 'in', employees_with_doj.ids)
            ])
            contract_emp_ids = set(existing_contracts.mapped('employee_id').ids)

            # Step 4: create missing contracts
            contracts_to_create = []
            for emp in employees_with_doj:
                if emp.id not in contract_emp_ids:
                    vals = emp._get_contract_vals_from_employee(emp)
                    contracts_to_create.append(vals)

            if contracts_to_create:
                self.env['hr.contract'].create(contracts_to_create)
