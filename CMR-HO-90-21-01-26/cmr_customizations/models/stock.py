            ...
            if picking.picking_type_id.code == 'incoming' and (picking.stock_type in ['ho_operation','sub_contract','data_import'] or is_subcontract):
                ...
                if move_lines:
                    ...
                    if (not picking.is_confirm and un_move_lines) or (
                        ...
                        if res and picking.purchase_id and picking.is_landed_cost == 'no' and picking.stock_type == 'ho_operation':
...
class StockMoveLine(models.Model):
    ...
    def lot_values_update(self):
        for move_line in self:
            if move_line.picking_id.picking_type_code == 'incoming' and move_line.picking_id.stock_type == 'ho_operation':
                move_line.lot_id.write({
                    'type_product': move_line.type_product,
...
class StockLogisticEntry(models.Model):
    ...
    def _check_logistic_qty_limit(self):
        """
        Prevent exceeding total logistic_qty across multiple pickings
        for the same logistic_id.
        """
        for rec in self:
            if rec.picking_id.purchase_id and rec.picking_id.stock_type == 'ho_operation' and rec.picking_id.stock_picking_type == 'receipt':
                if not rec.logistic_id:
                    continue

                if  0 > rec.logistic_act_qty:
                    raise ValidationError("You cannot give Negative Qty.")
                if  0 == rec.logistic_act_qty:
                    raise ValidationError("You cannot give Zero Qty.")
                # Sum up all existing logistic entries (excluding current record)
                total_used_qty = sum(self.search([
                    ('logistic_id', '=', rec.logistic_id.id),
                    ('id', '!=', rec.id)
                ]).mapped('logistic_act_qty'))
                # Include current record's entered qty
                total_used_qty += rec.logistic_act_qty
                if total_used_qty > rec.logistic_qty:
                    raise ValidationError(
                        f"The total used quantity ({total_used_qty}) exceeds "
                        f"the allowed logistic quantity ({rec.logistic_qty}) for Logistic No {rec.logistic_id.display_name}."
                    )
