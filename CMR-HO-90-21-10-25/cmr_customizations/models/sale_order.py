    ...
    def _onchange_barcode_scanned(self):
        ...
        if self.barcode_scanned:
            ...
            # EAN-13 Barcode
                ...
                location = self.env.ref('stock.stock_location_stock').id
                lots = self.env['stock.quant'].search([
                    ('lot_id.ref', '=', ean13_barcode),('location_id.id','=',location),
                    ('quantity', '>', 0),('lot_id.type_product','=','brand'),
                    ('company_id', '=', self.company_id.id)
                ], order='id asc')
                if not lots:
                ...
                for lot in lots.lot_id:
                    sale_serial_type = 'return' if lot.serial_type == 'return' else 'regular'
                    if lot.name in used_names and lot.product_id.tracking == 'lot':
                        raise ValidationError(f"Already Added {lot.name}")
                    if lot.name in used_names and lot.product_id.tracking == 'serial':
                        continue
                    if product.tracking == 'serial':
                ...
                replicated = available_lot.product_id.product_replication_list_id.filtered(
                    lambda x: x.store_id.name == self.partner_id.name)
                if available_lot.rs_price <= 0.0 :
                    raise ValidationError(f"You are not allowed to some of the serial numbers are not done landed cost.")
                if replicated and replicated.date_replication == False:
                    raise ValidationError(
                        f"This Article {available_lot.product_id.display_name} not integrated to store.")
                new_line = (0, 0, {
                    'product_id': product.id,
...
class SaleOrderLine(models.Model):
    ...
    def check_landed_cost_yes(self):
        for lot in self:
            if lot.lot_ids.picking_id and lot.lot_ids.picking_id.is_landed_cost == 'yes':
                landed_cost = self.env['stock.landed.cost'].sudo().search([('picking_ids.id', '=',lot.lot_ids.picking_id.id),('state','=','done')])
                if not landed_cost:
