...
class order_line_wizard(models.TransientModel):
    ...
    def action_import_barcodes(self):
        ...
        # Get already assigned lots/serials across all sale orders
        used_lot_ids = self.env['sale.order.line'].search([
            ('lot_ids', '!=', False),
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            # Determine barcode type and search domain
            if 'R' in barcode:
                # GS1 barcode logic
                idx = barcode.find('R')
            ...
            if product.tracking == 'serial':
                ...
                for q in available_quants:
                    ...
                    # Check if serial already used in any order
                    ...
                    if existing_line:
                        # GS1 barcode → stop immediately
                        if 'R' in barcode:
                            existing_type = existing_line.sale_serial_type or 'Undefined'
                            skipped_lines.append(
                                (barcode, f"Serial {lot.name} already used in order {existing_line.order_id.name} "
                                          f"with type '{existing_type}'")
                            )
                            created_count = 0
                            break
                        else:
                            # EAN-13 → skip this serial, move to next
                            continue

                    # Create order line
                    sale_serial_type = self.env['sale.order.line'].search([
                        ('order_id', '=', self.sale_order_id.id),
                        ('product_id', '=', product.id)
                    ], limit=1).sale_serial_type or 'regular'
                ...
                # If we couldn't create full quantity (EAN-13 case)
            ...
            # --- LOT-TRACKED PRODUCT ---
                ...
                # Detect sale_serial_type from existing order line for same product/lot
                existing_line_same_order = self.env['sale.order.line'].search([
                    ('order_id', '=', self.sale_order_id.id),
                    ('lot_ids', 'in', lot_ids),
                ], limit=1)
                current_serial_type = existing_line_same_order.sale_serial_type if existing_line_same_order else 'regular'

                # Compute already-used qty for same type
                already_used_qty = sum(
                ...
                remaining_qty_for_lot = total_available_qty - already_used_qty

                if remaining_qty_for_lot <= 0:
                    skipped_lines.append((barcode, f"Lot already fully used for '{current_serial_type}' type"))
                    continue
                if qty > remaining_qty_for_lot:
                    skipped_lines.append(
                        (barcode, f"Requested quantity {qty} exceeds remaining available {remaining_qty_for_lot} "
                                  f"for '{current_serial_type}' type")
                    )
                    continue
                ...
                for q in available_quants:
                    ...
                    lot_remaining = lot_total - lot_used
                    if lot_remaining <= 0:
                        continue
