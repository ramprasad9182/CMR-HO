...
class mrp_prod_wizard(models.TransientModel):
    _name = 'mrp.prod.line.wizard'
    _description = "MRP Components Wizard"

    mrp_prod_file = fields.Binary(string="Select File")
        ...
        if self.import_option == 'csv':
            ...
            try:
                wb = openpyxl.load_workbook(
                    filename=BytesIO(base64.b64decode(self.mrp_prod_file)), read_only=True
                )
                ws = wb.active
            except Exception:
            ...
            for row_no in ws.iter_rows(min_row=2, max_row=None, min_col=None,
                ...
                if values:
                    if len(row_no) == 0:
                        ...
                        res = self.create_mrp_prod_line(values)
        else:
            try:
                wb = openpyxl.load_workbook(
                    filename=BytesIO(base64.b64decode(self.mrp_prod_file)), read_only=True
    ...
    def create_mrp_prod_line(self, values):
        mrp_prod_brw = self.env['mrp.production'].browse(self._context.get('active_id'))
        serial_no = self.env['stock.lot']
        product_obj_search = self.env['product.product']
        location_dest_id = self.env['stock.location'].search([('usage','=','production'),('company_id','=',self.env.user.company_id.id)])
        if self.product_details_option == 'from_product':
            ...
            if mrp_prod_brw.state == 'draft':
                ...
                existing_move_line = mrp_prod_brw.move_raw_ids.filtered(
                    lambda x: x.product_id == product_id)
                if existing_move_line:
                    ...
                    move_lines = self.env['stock.move'].sudo().create({
                        'raw_material_production_id': mrp_prod_brw.id,
                        ...
                        'location_id': mrp_prod_brw.location_src_id.id,
                        'location_dest_id': location_dest_id.id,
                    })
            elif mrp_prod_brw.state != 'draft':
            ...
            if mrp_prod_brw.state == 'draft':
                if product_id.nhcl_product_type == 'unbranded':
                    existing_lot_line = mrp_prod_brw.move_raw_ids.filtered(
                        lambda x: x.product_id == product_id)
