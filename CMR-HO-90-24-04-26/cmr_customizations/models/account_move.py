...
class AccountMove(models.Model):
    ...
    def _compute_irn_ewaybill(self):
        for move in self:
            move.irn_number = False
            move.ewaybill_number = False
            ...
            try:
                raw = edi_doc.sudo().attachment_id.raw.decode('utf-8')

                # ✅ Extract only JSON part (ignore garbage)
                ...
                move.ewaybill_number = (
                                               data.get('EwbNo')
                                               or data.get('ewayBillNo')
                                       ) or False
