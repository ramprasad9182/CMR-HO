...
class LogisticScreen(models.Model):
    ...
    _rec_name = 'lr_number'
    _description = "Logistic Screen Data"
    ...
    #     res = super(LogisticScreen, self).default_get(fields_list)
    #     logistic_type = self.env.context.get('logistic_entry_types', 'manual')
    ...
    # @api.constrains('lr_number')
    # def _check_unique_lr_number(self):
    #     for record in self:
    #         existing_records = self.env['logistic.screen.data'].search(
    #             [('lr_number', '=', record.lr_number), ('id', '!=', record.id)])
    #         if existing_records:
    #             raise ValidationError('The Way LR Number Must Be Unique!.')
    ...
    def create(self, vals_list):
        ...
        existing_names = set(
            self.env['logistic.screen.data'].search([]).mapped('name')
        )
        def get_unique_sequence():
            # prevent infinite loop
            for _ in range(10):
                seq_number = seq.next_by_code('logistic.screen.data') or 'New'
                if seq_number not in existing_names:
                    existing_names.add(seq_number)
                    return seq_number
            raise ValidationError("Unable to generate unique sequence after multiple attempts.")
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == 'New':
                vals['name'] = get_unique_sequence()
    ...
    def action_confirm(self):
        ...
        elif self.way_bill_no == False and self.po_number.amount_total > 50000:
            raise ValidationError("Enter alpha numeric E-Way Bill Numbe")
