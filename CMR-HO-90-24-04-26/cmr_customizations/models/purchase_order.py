...
class PurchaseOrder(models.Model):
    ...
    nhcl_po_type = fields.Selection(
        [('advertisement', 'Advertisement'),('direct_po','Direct PO'), ('data_import', 'Data Import'),('ho_operation', 'HO Operation'), ('sub_contract', 'Sub Contracting'), ('inter_state','Inter State'), ('intra_state', 'Intra State'), ('others', 'Others')],
    ...
    is_vendor_change = fields.Selection([('yes', 'Yes'),
                                         ('no', 'No')], string="Vendor Change", copy=False, tracking=True, default='no')

    @api.constrains('is_vendor_change')
    def check_is_vendor_change(self):
        if (self.is_vendor_change == 'yes' or self.is_vendor_change == 'no') and self.nhcl_po_type not in ['direct_po',
                                                                                                           'ho_operation'] and not self.partner_id.name.startswith('CMR') :
            raise ValidationError('Vendor Change should not touch.')



    def button_import_po(self):
        return {
            "name": _("Import PO"),
    ...
    def onchange_category_based_on_current_user_id(self):
        if self.user_id:
            categ_ids = self.env['product.category'].search([('user_ids', 'in', self.user_id.ids)])
            category_lst = []
    ...
    def select_purchase_order_product_ids(self):
        allowed_ids = []
        ...
        return {
            ...
            'type': 'ir.actions.act_window',
            'res_model': 'multi.purchase.product.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def select_purchase_product_direct_ids(self):
        allowed_ids = []
        for allowed_category_id in self.allowed_category_ids:
            categ_ids = self.env['product.category'].search([('parent_id', 'ilike', allowed_category_id.name)])
            categ_ids_lst = []
        ...
        return {
            'name': 'Select Products',
            'type': 'ir.actions.act_window',
            'res_model': 'multi.purchase.product.wizard.direct',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'default_purchase_order_id': self.id,
    ...
    def _onchange_nhcl_po_type(self):
        for i in self:
            if i.nhcl_po_type == 'ho_operation':
                ...
                i.dummy_po_type = 'ho_operation'
            elif i.nhcl_po_type == 'direct_po':
                i.dummy_po_type = 'ho_operation'
            elif i.nhcl_po_type == 'data_import':
                i.dummy_po_type = 'ho_operation'
            else:
                i.dummy_po_type = ''
    ...
    def verify_draft_pi_to_po(self):
        for rec in self:
            if rec.origin and not rec.requisition_id and rec.state not in ['draft', 'sent'] and rec.nhcl_po_type == 'ho_operation':
    ...
    def button_confirm(self):
        ...
        for order in self:
            ...
            if order.nhcl_po_type == 'data_import' and order.picking_ids:
                moves = order.picking_ids.mapped('move_ids_without_package')
                for line in order.order_line:
                    move = moves.filtered(lambda m: m.purchase_line_id.id == line.id)
                    if move:
                        move.write({
                            'type_product': line.type_product,
                            'move_brand_barcode': line.move_brand_barcode,
                            'move_cp': line.move_cp,
                            'move_mrp': line.move_mrp,
                            'move_rsp': line.move_rsp,
                        })
            approval = self.env['approval.request'].sudo().search([('name', '=', 'origin')])
            if approval and order.is_vendor_change == 'yes':
                approval.partner_id = order.partner_id.id
                for po_line, pi_line in zip(order.order_line, approval.product_line_ids):
                    pi_line.write({
                        'unit_price': po_line.price_unit,
                        'default_rsp_margin': po_line.default_rsp_margin,
                        'pi_rsp_price': po_line.rsp,
                        'pi_mrp_price': po_line.mrp,
                        'enter_rsp_margin': po_line.purchase_rsp_margin,
                    })
        return res
    ...
    def button_approve(self, force=False):
        ...
        for order in self:
            logistic = self.env['logistic.screen.data'].search_count(
            [('po_number', '=', self.id)])
            if order.nhcl_po_type == 'ho_operation' and logistic < 1:
                ...
                if len(unique_zone_ids) == 1:
                    ...
                    if zone_id:
                        ...
                        self.env['logistic.screen.data'].create({
                            ...
                            'transporter': order.transporter.id
                        })
            approval = self.env['approval.request'].sudo().search([('name', '=', 'origin')])
            if approval and order.is_vendor_change == 'yes':
                approval.partner_id = order.partner_id.id
    ...
    def _prepare_picking(self):
        ...
        return {
            ...
            'logistic_vendor': self.logistic_vendor.id,
...
class PurchaseOrderLine(models.Model):
    ...
    nhcl_pi_qty = fields.Float(string="PI Qty", copy=False, compute='get_pi_qty')
    ...
    def onchange_mrp(self):
        for rec in self:
            if rec.rsp > 0 and rec.mrp and rec.rsp >= rec.mrp:
                raise ValidationError('RSP Price should not be greater then MRP Price')

    @api.onchange('price_unit', 'rsp')
        ...
        for rec in self:
            if rec.product_id:
                ...
                if rec.nhcl_margin_rsp_type == 'rsp':
                    ...
                    if len(tax_ids) > 1:
                        for tax in tax_ids:
                            if tax.min_amount <= rec.rsp <= tax.max_amount:
                                temp_price = rec.rsp * (tax.amount / 100.0)
                                tax_temp_price = rec.rsp - temp_price
                                rec.purchase_rsp_margin = ((tax_temp_price - rec.price_unit) / rec.price_unit) * 100
                                break
                    else:
                        if tax_ids and rec.price_unit > 0:
                            temp_price = rec.rsp * (tax_ids.amount / 100.0)
                            tax_temp_price = rec.rsp - temp_price
                            rec.purchase_rsp_margin = ((tax_temp_price - rec.price_unit) / rec.price_unit) * 100
                        else:
                            if rec.price_unit > 0:
                                rec.purchase_rsp_margin = ((rec.rsp - rec.price_unit) / rec.price_unit) * 100
    ...
    def onchange_default_rsp_margin(self):
        for rec in self:
            ...
            if margin_lines and rec.product_id and rec.price_unit > 0:
                matched = False
                for line in margin_lines:
                    if line.from_range <= rec.price_unit <= line.to_range:
                ...
                if not matched:
                    raise ValidationError(
                        ...
                        (rec.price_unit, rec.product_id.categ_id.parent_id.parent_id.parent_id.name))

    @api.onchange('purchase_rsp_margin')
    def onchange_purchase_rsp_margin(self):
        for rec in self:
            temp_price = 0.0
            if rec.nhcl_margin_rsp_type == 'margin':
                base_price = rec.price_unit or 0
                margin_price = (rec.purchase_rsp_margin / 100) * base_price if rec.purchase_rsp_margin else 0
    ...
    def onchange_mrp(self):
        ...
        for rec in self:
            ...
            if self.env['product.tag'].search([('name', 'in', ['Readymade', 'Footware'])]):
                if rec.product_id and rec.product_id.product_tag_ids:
                    ...
                    if tags.filtered(lambda tag: tag.name.capitalize() == 'Readymade'):
                        if 1050 < rec.rsp < 1200:
                            raise ValidationError(
                                'For ReadyMade products, RSP price must not be between 1050 and 1200.')
                else:
                    raise ValidationError(f"Product Template tag is not available for {rec.product_id.name}")
            else:
                raise ValidationError("Readymade,Footware are not created in the product template tags")

            if not rec.rsp or rec.rsp <= 0:
            ...
            for line in mrp_lines:
                if line.from_range <= rec.rsp <= line.to_range:
                    margin = line.margin or 0
                    if margin != 0:
                        margin_price = (margin / 100.0) * rec.rsp
                        temp_price = round(rec.rsp + margin_price)
                        if temp_price:
                            last_two = temp_price % 100
                            if last_two < 50:
                                rec.mrp = temp_price - last_two + 49
                            else:
                                rec.mrp = temp_price - last_two + 99
                    return
            return {
                'warning': {
    ...
    def _compute_tax_id(self):
        for line in self:
            ...
            if line.order_id.nhcl_po_type in ['intra_state','data_import']:
                line.taxes_id = False
                continue
