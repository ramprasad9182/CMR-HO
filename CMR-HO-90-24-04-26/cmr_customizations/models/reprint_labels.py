...
class ReprintLabels(models.Model):
    ...
    @api.onchange('grc_number')
    def _onchange_grc_number(self):
        if not self.grc_number:
            return

        input_value = self.grc_number.strip()
        ...
        if not picking:
            self.serial_number_lines = [(5, 0, 0)]
            return

        # =====================================================
        # 🔹 Get all move lines under that GRC
        # =====================================================
        ...
        # =====================================================
        # 🔹 Build lines (all barcodes / serials)
        # =====================================================
        self.serial_number_lines = [(5, 0, 0)]  # clear old lines

        for ml in move_lines:
            lot = ml.lot_id or Lot.search([
                ('name', '=', ml.lot_name),
                ('product_id', '=', ml.product_id.id)
            ], limit=1)

            vals = {
                'name': lot.name if lot else ml.lot_name,
                'product_id': ml.product_id.id,
                'lot_id': lot.id if lot else False,
                'internal_ref': ml.internal_ref_lot,
                'qty': ml.qty_done or 1,
                'category_1': lot.category_1.id if lot and lot.category_1 else False,
                'category_2': lot.category_2.id if lot and lot.category_2 else False,
                'category_3': lot.category_3.id if lot and lot.category_3 else False,
                'category_4': lot.category_4.id if lot and lot.category_4 else False,
                'category_5': lot.category_5.id if lot and lot.category_5 else False,
                'category_6': lot.category_6.id if lot and lot.category_6 else False,
                'category_7': lot.category_7.id if lot and lot.category_7 else False,
                'category_8': lot.category_8.id if lot and lot.category_8 else False,
                'description_1': lot.description_1.id if lot and lot.description_1 else False,
                'description_2': lot.description_2.id if lot and lot.description_2 else False,
                'description_3': lot.description_3.id if lot and lot.description_3 else False,
                'description_4': lot.description_4.id if lot and lot.description_4 else False,
                'description_5': lot.description_5.id if lot and lot.description_5 else False,
                'description_6': lot.description_6.id if lot and lot.description_6 else False,
                'description_7': lot.description_7.id if lot and lot.description_7 else False,
                'description_8': lot.description_8.id if lot and lot.description_8 else False,
                'description_9': lot.description_9.id if lot and lot.description_9 else False,
                'product_description': lot.product_description if lot else '',
                'web_product': lot.web_product if lot else '',
                'cost_price': lot.cost_price if lot else 0.0,
                'actual_cp': lot.actual_cp if lot else 0.0,
                'mr_price': lot.mr_price if lot else 0.0,
                'rs_price': lot.rs_price if lot else 0.0,
            }

            lines.append((0, 0, vals))

        self.serial_number_lines = lines
        self.grc_number = False
...
# 09-02
    ...
    def _onchange_serial_number_lines(self):
        ...
        input_value = self.serial_numbers.strip()
        MoveLine = self.env['stock.move.line']
        lines = []
        Product = self.env['product.product']

        # =====================================================
        # 1️⃣ Try to find a move line by internal_ref (brand)
        # =====================================================
        move_lines = MoveLine.search([
            ('type_product', '=', 'brand'),
            ('internal_ref_lot', '=', input_value),
        ], limit=50)
        ...
        # =====================================================
        # 2️⃣ If not found, try unbrand by lot_id or lot_name
        # =====================================================
        if not move_lines:
            move_lines = MoveLine.search([
                ('type_product', '=', 'unbrand'),
                '|',
                ('lot_id.name', '=', input_value),
                ('lot_name', '=', input_value),
            ], limit=50)

        # =====================================================
        # 3️⃣ If still nothing, fallback to direct lot search
        # =====================================================
        if not move_lines:
            lot = self.env['stock.lot'].search([('name', '=', input_value)], limit=1)
            if lot:
                vals = {
                    'name': lot.name,
                    'product_id': lot.product_id.id,
                    'lot_id': lot.id,
                    'internal_ref': getattr(lot, 'ref', ''),
                    'qty': 1,
                    'category_1': lot.category_1.id if lot.category_1 else False,
                    'category_2': lot.category_2.id if lot.category_2 else False,
                    'category_3': lot.category_3.id if lot.category_3 else False,
        ...
        # =====================================================
        # 4️⃣ If move lines found, build serial_number_lines
        # =====================================================
        self.serial_number_lines = [(5, 0, 0)]  # clear old lines
        for ml in move_lines:
            # Use the lot from move line, or fallback by lot_name
            lot = ml.lot_id or self.env['stock.lot'].search([
                ('name', '=', ml.lot_name),
                ('product_id', '=', ml.product_id.id)
            ], limit=1)
...
class SerialNumberLines(models.Model):
    ...
    _description = 'Serial Numner Lines'
