...
class StockAssignSerialNumbers(models.TransientModel):
    ...
    def apply(self):
        if self.production_id:
            check = self.production_id.move_raw_ids.filtered_domain([('lot_ids','=',False)])
            if check:
                raise ValidationError(_('Serial Numbers are not added to all components.'))
        ...
        for move in production.move_raw_ids:
            ...
            expected_qty = round(((bom_line.product_qty / bom.product_qty) * self.produced_qty), 2)
            consumed_qty = move.product_uom_qty
            if expected_qty != consumed_qty:
                mismatch.append(
                    f"{move.product_id.display_name}: "
                    f"Required {expected_qty}, Consumed {consumed_qty}"
                )
        if mismatch:
            raise ValidationError(
                "Component quantity mismatch:\n" + "\n".join(mismatch)
            )
        res = super(StockAssignSerialNumbers,self).apply()
        return res
