...
class GETJournals(http.Controller):
    ...
    def get_vendor_bill_journal_entries(self, **kwargs):
        ...
        data = request.env['account.move'].sudo().search([('move_type', 'in', ['in_invoice']),('nhcl_flag', '=', 'n'),('state', '=','posted')])
    ...
    def get_vendor_payment_journal_entries(self, **kwargs):
        ...
        if not integration:
            return request.make_response(
                '<transactions>Integration configuration not done</transactions>',
                headers=[('Content-Type', 'application/xml')]
            )
        ...
        if not api_key or api_key != integration.api_key:
            return request.make_response(
                '<transactions>Invalid API key</transactions>',
                headers=[('Content-Type', 'application/xml')]
            )

        if not integration.journal_entries:
            return request.make_response(
                '<transactions>No journal types enabled</transactions>',
                headers=[('Content-Type', 'application/xml')]
            )
        ...
        if not payments:
            return request.make_response(
                '<transactions>No vendor payments found</transactions>',
                headers=[('Content-Type', 'application/xml')]
            )
        ...
        for pay in payments:
            ...
            if not move or move.state != 'posted' or move.nhcl_flag != 'n':
                continue

            # Narration clean
            narration = False
            if move.narration:
                soup = BeautifulSoup(move.narration, 'html.parser')
                narration = soup.get_text()
        ...
        if not result:
            return request.make_response(
                '<transactions>No valid vendor payment journal entries found</transactions>',
                headers=[('Content-Type', 'application/xml')]
            )

        # XML conversion (your function)
        xml_data = json_to_xml(result, root_tag="transactions")
    ...
    @http.route('/odoo/get_customer_payment_journal_entries', type='http', auth='public', methods=['GET'])
    def get_customer_payment_journal_entries(self, **kwargs):
        integration = request.env['tally.integration'].sudo().search([('active_record', '=', True)], limit=1)
        ...
        if not payments:
            return request.make_response(
                '<transactions>No customer payments found</transactions>',
                headers=[('Content-Type', 'application/xml')]
            )
        ...
        if not result:
            return request.make_response(
                '<transactions>No valid customer payment journal entries found</transactions>',
                headers=[('Content-Type', 'application/xml')]
            )
