...
class PosOrderController(http.Controller):
    @http.route('/api/pos.order/call_action', type='json', auth='public', methods=['POST'],csrf=False)
    def pos_order_invoice(self,**kwargs):
        ho_store_id = request.env['nhcl.ho.store.master'].sudo().search(
            [('nhcl_store_type', '=', 'ho'), ('nhcl_active', '=', True), ])
        store_pos_orders = request.env['pos.order'].sudo().search([('nhcl_store_je', '=', True),('state','=','paid')], limit=3)
        if store_pos_orders:
            for order in store_pos_orders:
                try:
                    if order:
