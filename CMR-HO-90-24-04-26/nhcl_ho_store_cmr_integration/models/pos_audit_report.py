...
class NhclPosAuditReport(models.Model):
    ...
    name = fields.Char(string='Name', default='Store POS Audit Report')

    @api.constrains('from_date', 'to_date')
    def _check_date_range(self):
        for record in self:
            if (record.from_date and not record.to_date) or (record.to_date and not record.from_date):
    ...
    def compute_company_domain(self):
        for rec in self:
            if rec.nhcl_store_id:
                ...
                domain = [('company_id', 'in', company_ids), ('state', '=', 'done')]
            else:
                domain = [('id', '=', 0)]
    ...
    def get_store_audit_plans(self):
        ...
        existing_refs = set(existing_records.mapped('plan_ref'))
        ...
        for store in self.nhcl_store_id:
            ...
            for data in store_audit_type_data_list:
                ...
                existing_refs.add(plan_ref)  # Update to avoid duplicates in next loops
    ...
    def get_pos_audit_report(self):
        ...
        if not self.audit_type_id:
            raise ValidationError("Please select the Audit Plan")
        ...
        for store in self.nhcl_store_id:
            ...
            audit_plan_names = self.audit_type_id.mapped('plan_name') if self.audit_type_id else []
            domain = [('state', '=', 'done')]
    ...
    def get_pos_audit_report_from(self):
        if not self.nhcl_store_id:
            raise ValidationError('Please Select the Company/Store.')
        ...
        for store in self.nhcl_store_id:
            # Base domain (mandatory)
            domain = [
                ('state', '=', 'done'),
                ('company_id', '=', store.company_id.id),
            ]
            # Audit Plan filter (optional)
            if self.audit_type_id:
                domain.append(('audit_plan_id', '=', self.audit_type_id.id))
                # If plan is stored as name instead of M2O, use:
                # domain.append(('plan_name', '=', self.audit_type_id.name))
            ...
            # Fetch inventories from HO
            inventories = self.env['stock.inventory'].search(domain)

            _logger.info(
                "POS Audit | Store: %s | Inventories: %s | Domain: %s",
                store.name, len(inventories), domain
            )
            ...
            for inv in inventories:
                for line in inv.inventory_line_ids:
                    ...
                    book_qty = line.theoretical_qty or 0.0
                    phy_qty = line.qty_done or 0.0
                    diff_qty = phy_qty - book_qty

                    data = {
                        'barcode': lot.name,
                        'item_name': lot.name,
                        'items_name': lot.product_id.name,
                        'article_name': lot.product_id.name,
                        'book_qty': book_qty,
                        'phy_qty': phy_qty,
                        'difference_qty': diff_qty,
                        'mrp_price': lot.mr_price or 0.0,
                        'rsp_price': lot.rs_price or 0.0,
                        'book_rsp_price': lot.rs_price or 0.0,
                        'phy_rsp_price': lot.rs_price or 0.0,
                        'difference_rsp_price': 0.0,
                        'cp_value': lot.cost_price or 0.0,
                        'book_cp_value': lot.cost_price or 0.0,
                        'phy_cp_value': lot.cost_price or 0.0,
        ...
        # Assign all report lines
        self.nhcl_pos_audit_ids = audit_lines

        _logger.info("POS Audit Report | Total lines created: %s", total_created)


    def action_stor_pos_audit_report_line(self):
        self.ensure_one()

        return {
            'type': 'ir.actions.act_window',
            'name': 'POS Audit Report',
            'res_model': 'nhcl.pos.audit.report.line',
            'view_mode': 'tree,pivot',
            'domain': [('nhcl_pos_audit_id', '=', self.id)],
            'context': {
                'default_nhcl_pos_audit_id': self.id
            }
        }
...
class NhclAuditPlan(models.Model):
    ...
    nhcl_sequence = fields.Char('NHCL Sequence', readonly=True)
