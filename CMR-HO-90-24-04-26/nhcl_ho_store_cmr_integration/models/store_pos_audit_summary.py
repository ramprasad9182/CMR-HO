...
class NhclStorePosAuditSummary(models.Model):
    ...
    name = fields.Char(string='Name', default='Store POS Audit Summary Report')
    ...
    def get_store_audit_plans(self):
        ...
        existing_refs = set(existing_records.mapped('nhcl_sequence'))
        ...
        for store in self.nhcl_store_id:
            ...
            for data in store_audit_type_data_list:
                ...
                nhcl_seq = data.get('nhcl_sequence')
                ...
                audit_plan_values.append({
                    ...
                    'nhcl_sequence': nhcl_seq,
                })
                existing_refs.add(nhcl_seq)  # Update to avoid duplicates in next loops
    ...
    def action_store_pos_audit_summary_report_line(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': 'POS Audit Summary Report',
            'res_model': 'nhcl.store.pos.audit.summary.line',
            'view_mode': 'tree,pivot',
            'domain': [('nhcl_store_pos_audit_summary_id', '=', self.id)],
            'context': {
                'default_nhcl_store_pos_audit_summary_id': self.id
            }
        }
