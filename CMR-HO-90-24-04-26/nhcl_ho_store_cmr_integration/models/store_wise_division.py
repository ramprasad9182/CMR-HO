...
class StoreWiseData(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == 'New':
                vals['name'] = seq.next_by_code('store.wise.data') or 'New'
    ...
    def get_store_division_data(self):
        ...
        to_dt = Datetime.to_datetime(self.from_date) + timedelta(days=1)
        # Step 1: Get all stock.lot records for the selected store/company
        stock_lots = self.env['stock.lot'].sudo().search([('company_id.id', '=', self.store_id.id),('product_qty','>',0),('create_date', '<', to_dt)])
        # Get unique division names
        division_names = set()
        for lot in stock_lots:
            division_name = lot.product_id.categ_id.parent_id.parent_id.parent_id.name
            if division_name:
