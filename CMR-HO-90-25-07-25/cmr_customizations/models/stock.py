...
class Picking(models.Model):
        ...
        for picking in self:
            ...
            if picking.picking_type_id.code == 'incoming' and (picking.stock_type in ['ho_operation','sub_contract','data_import'] or is_subcontract):
                ...
                for line in picking.move_ids_without_package:
                    ...
                    #     raise ValidationError(f"Please Select The Tracking Type In Product.'{line.product_id.name}'")
                for move_line in picking.move_line_ids_without_package:
                    if move_line.type_product == 'brand' and move_line.internal_ref_lot == '':
                    ...
                    if (not picking.is_confirm and un_move_lines) or (
                        ...
                        if res and picking.purchase_id and picking.is_landed_cost == 'no':
                            for move_line in picking.move_ids_without_package.move_line_ids:
                                ...
                                lot_id.actual_cp = purchase_line_id.price_unit
                                lot_id.cost_price = purchase_line_id.price_unit
                                lot_id.category_8 = purchase_line_id.purchase_category_id.id
                                lot_id.ho_grc_no = picking.name
                                # lot_id.get_margin_from_category()
                                lot_id.nhcl_margin_lot = move_line.approval_margin
                                lot_id.calculate_rsp_price()
                                lot_id.get_mrp_margin_from_category()
                                lot_id._get_product_aging()
                                move_line.cost_price = purchase_line_id.price_unit
                                move_line.categ_8 = lot_id.category_8.id
                                move_line.rs_price = lot_id.rs_price
                                move_line.mr_price = lot_id.mr_price
                ...
                for line in picking.move_line_ids:
                    if line.lot_id:
                        ...
                        line.lot_id.product_qty = 1
                        line.lot_id.cost_price = cost_price.cost_price
                        line.lot_id.type_product = cost_price.type_product
    ...
    def _onchange_stock_barcode(self):
        ...
        # if self.state != 'done':
        #     raise ValidationError(f"You cannot add the products, once it is in {self.state}.....")
        if self.stock_barcode:
...
class StockMoveLine(models.Model):
    ...
    barcode_readonly = fields.Boolean(compute='making_readonly')
    ...
    def making_readonly(self):
        for rec in self:
            if rec.picking_id and rec.picking_id.state == 'done':
                rec.barcode_readonly = True
            else:
                rec.barcode_readonly = False
...
class StockLot(models.Model):
    ...
    # product_aging = fields.Many2one('product.aging.line', string="Dummy Product Aging",
    #                                 copy=False)
    ...
    # @api.depends('name')
    # def get_margin_from_category(self):
    #     for rec in self:
    #         rec.segment = rec.product_id.segment
    #         if rec.product_id.product_tmpl_id.categ_id.parent_id.parent_id.parent_id.product_category_margin_ids:
    #             for line in rec.product_id.product_tmpl_id.categ_id.parent_id.parent_id.parent_id.product_category_margin_ids:
    #                 if line.from_range <= rec.cost_price <= line.to_range:
    #                     rec.nhcl_margin_lot = line.margin
    #                     break
    #                 else:
    #                     rec.nhcl_margin_lot = 0
    #             margin = self.env['product.margin.approval.line'].search([('lot_id.name', '=', rec.name)],
    #                                                                      order="id desc", limit=1)
    #             if margin and margin.product_margin_approval_id.state == 'approved':
    #                 rec.nhcl_margin_lot = margin.new_margin
    #                 rec.shortage = margin.shortage
    #         else:
    #             rec.nhcl_margin_lot = 0
    #         rec.calculate_rsp_price()
    ...
    def calculate_rsp_price(self):
        for rec in self:
            if rec.nhcl_margin_lot != 0:
                ...
                temp_price = round(rec.cost_price + margin_price)
            else:
            ...
            for line in cost.valuation_adjustment_lines.filtered(lambda line: line.move_id):
                for move_line in line.move_id.move_line_ids:
                    if move_line.lot_id:
                        ...
                        lot.cost_price = line.nhcl_cost_price
    ...
    def compute_cost_price_per_unit(self):
        for rec in self:
            if rec.final_cost and rec.quantity:
                rec.nhcl_cost_price = rec.final_cost/rec.quantity
...
class StockVerificationImport(models.TransientModel):
    ...
    #
    #                     move_line = move_lines.filtered(lambda ml: ml.lot_id.name == serial_no)
    #                     if not move_line:
    ...
    #                     lambda ml: ml.internal_ref_lot == barcode and ml.lot_id.name not in found_serials)
    #                 available_serials = [ml.lot_id.name for ml in matching_lines]
    #
