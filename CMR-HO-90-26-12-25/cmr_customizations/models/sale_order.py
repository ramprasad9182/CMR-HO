...
class SaleOrder(models.Model):
    ...
    def _onchange_barcode_scanned(self):
        ...
        # ------------------------------------------
            ...
            allocated_lines = []
            allocated_any = False  # <-- IMPORTANT FIX

            # --------------------------
            # LOOP OVER ALL LOTS (FIFO)
            # --------------------------
            for quant in lots:
                lot = quant.lot_id
                sale_serial_type = 'return' if lot.serial_type == 'return' else 'regular'
                ...
                # ------------------------------
                # LOT PRODUCT HANDLING (FIXED)
                # ------------------------------
                # Existing line for this lot?
                existing_line = self.order_line.filtered(
                    lambda l: l.sale_serial_type == sale_serial_type
                              and l.product_id.id == product.id
                              and lot.id in l.lot_ids.ids
                )[:1]
                # Compute total availability for this lot
                existing_qty = global_lot_qty(lot, sale_serial_type)
                total_stock_qty = sum(
                ...
                available_qty = total_stock_qty - existing_qty - existing_line.product_uom_qty - vendor_return_qty
                # Skip lots with no stock available
                if available_qty <= 0:
                    continue
                # ------------------------------------------
                # CASE 1: Merge into existing line (partial OK)
                # ------------------------------------------
                if existing_line:
                    ...
                    # If still need qty, move to next lot
                    if remaining_qty > 0:
                        continue
                    break  # fully allocated
                # ------------------------------------------
                # CASE 2: Create NEW line for this lot
                # ------------------------------------------
                allocate_qty = min(remaining_qty, available_qty)
                if allocate_qty <= 0:
                    continue
        ...
        # ------------------------------------------
            ...
            for lot in lots.lot_id:
                ...
                # Lot product (MERGE HERE)
                    ...
                    out_lot_qty = self.env['stock.move.line'].sudo().search([
                        ('company_id.nhcl_company_bool', '=', True),
                        ('lot_id', '=', lot.id),('picking_id.stock_picking_type','=','goods_return')
                    ])

                    if existing_qty + self.entered_qty <= (sum(lot_qty.mapped('quantity')) - sum(out_lot_qty.mapped('quantity'))):
