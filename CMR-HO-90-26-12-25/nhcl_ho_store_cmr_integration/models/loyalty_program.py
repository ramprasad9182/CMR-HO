    ...
    def update_loyalty_program_data(self):
        for line in self.loyalty_program_id:
            ...
            try:
                ...
                rule_line = []
                rule_count = 0
                for (index, data) in zip(range(0,len(self.rule_ids)),self.rule_ids):
                    category_1_lst = []
                    category_2_lst = []
                    category_3_lst = []
                    category_4_lst = []
                    ...
                    # all_existing_value = next((val for val in rule_value_data if
                    #                        val.get('program_id')['id'] == program_id),
                    #                       None)
                    rule_count +=1
                    if rule_count <= rule_length:
                        existing_value = rule_value_data[index]
                ...
                reward_line = []
                reward_count = 0
                for (rew_index,reward) in zip(range(0,len(self.reward_ids)),self.reward_ids):
                    # Check if the value already exists in destination (reward_value_data)
                    # existing_reward_value = next(
                    #     (val1 for val1 in reward_value_data if
                    #      val1.get('discount') == reward.discount and val1.get('reward_type') == reward.reward_type),
                    #     None)

                    discount_line_product_id = False
                    reward_count +=1
                    if reward.discount_line_product_id:
                        discount_line_product_id = reward.discount_line_product_id.get_store_products(ho_ip, ho_port,headers_source)
                    if reward_count <= reward_length:
                        existing_reward_value = reward_value_data[rew_index]
