    ...
    def get_last_first_bill_num(self):
        ...
        try:
            ...
            for store in self.nhcl_store_id:
                ...
                # user_tz = self.env.user.tz or pytz.timezone('Asia/Kolkata')
                local = pytz.timezone('Asia/Kolkata')

                # Format dates as YYYY-MM-DD
                from_date_local = datetime.strptime(
                    str(self.from_date), DEFAULT_SERVER_DATETIME_FORMAT
                )
                ...
                to_date_local = datetime.strptime(
                    str(self.to_date), DEFAULT_SERVER_DATETIME_FORMAT
                )
                to_date_local = local.localize(to_date_local)

                # Format as DD/MM/YY HH:MM:SS
                from_date_str = from_date_local.strftime("%d/%m/%y %H:%M:%S")
                to_date_str = to_date_local.strftime("%d/%m/%y %H:%M:%S")
