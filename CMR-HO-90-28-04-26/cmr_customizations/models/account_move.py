...
class AccountMove(models.Model):
    ...
    irn_number = fields.Char(compute="_compute_irn_ewaybill", store=False, copy=False)
    ewaybill_number = fields.Char(compute="_compute_irn_ewaybill", store=False, copy=False)

    @api.depends('attachment_ids')
    def _compute_irn_ewaybill(self):
        for move in self:
            irn = False
            ewaybill = False
            ...
            for att in move.attachment_ids:
                ...
                if not att.name or not att.name.lower().endswith('.json'):
                    continue

                # 🔥 read real content
                raw = att._file_read(att.store_fname)
                if not raw:
                    continue
                ...
                # -------- IRN --------
                irn = data.get('Irn') or irn

                # nested "data"
                if not irn and isinstance(data.get('data'), str):
                    try:
                        irn = json.loads(data['data']).get('Irn')
                    except Exception:
                        pass

                # JWT (SignedInvoice)
                if not irn and data.get('SignedInvoice'):
                    try:
                        part = data['SignedInvoice'].split('.')[1]
                        part += '=' * (-len(part) % 4)
                        payload = json.loads(base64.b64decode(part).decode())
                        irn = json.loads(payload.get('data', '{}')).get('Irn')
                    except Exception:
                        pass

                # -------- EWAY --------
                if not ewaybill:
                    ewb = data.get('EwbNo') or data.get('ewayBillNo')
                    if ewb:
                        ewaybill = str(ewb)
    ...
    #
    #             if 'einvoice' not in name and 'ewaybill' not in name:
    #                 continue
    #             try:
    #                 data = json.loads(base64.b64decode(att.datas))
    #             except Exception:
    #                 continue
    #
    #             # -------- IRN --------
    #             if 'einvoice' in name and not irn:
    #                 irn = data.get('Irn') or data.get('data', {}).get('Irn')
    #
    #             # -------- EWAY BILL --------
    #             elif 'ewaybill' in name and not ewaybill:
    #                 ewb = (
    #                         data.get('EwbNo') or
    #                         data.get('ewayBillNo') or
    #                         data.get('data', {}).get('EwbNo')
    #                 )
    #
    #                 # handle null / False
    #                 if ewb:
    ...
    def create(self, vals_list):
        ...
        # Step 1: prefetch partners once
        customer_partners = partner_model.search([
            ('group_contact.name', 'in', ['Customer', 'Branch'])
        ])
        vendor_partners = partner_model.search([
        ...
        for move in moves:
            ...
            if move.move_type in ['out_invoice', 'out_refund', 'entry']:
                move.ref_partner_ids = customer_partners

            elif move.move_type in ['in_invoice', 'in_refund', 'in_receipt']:
                move.ref_partner_ids = vendor_partners

            # ---- reference update ----
            if move.reversed_entry_id and move.reversed_entry_id.ref:
                move.ref = f"{move.ref or ''} , {move.reversed_entry_id.ref}"

            # ---- TAX SYNC ----
            selected_taxes = set(
                move.tds_tax_ids.ids +
                move.tcs_tax_ids.ids +
                move.cess_tax_ids.ids
            )

            for line in move.line_ids.filtered(
                    lambda l: l.display_type not in ('line_section', 'line_note')
            ):
                current_tax_ids = set(line.tax_ids.ids)

                # compute final taxes (single operation)
                final_tax_ids = current_tax_ids.union(selected_taxes)

                # remove unwanted managed taxes
                managed_existing = line.tax_ids.filtered(
                    lambda t: t.nhcl_tax_type in ['tds', 'tcs', 'cess']
                ).ids

                final_tax_ids = set(final_tax_ids) - set(
                    t for t in managed_existing if t not in selected_taxes
                )

                # assign once (important optimization)
                line.tax_ids = [(6, 0, list(final_tax_ids))]
    ...
    def invoice_odt_report(self):
        ...
        report_xml_id = 'cmr_customizations.text_tiles_invoice_report_odt'

        if self.partner_id and self.env.company.state_id:
            if self.partner_id.state_id.id == self.env.company.state_id.id:
                report_xml_id = 'cmr_customizations.text_tiles_invoice_report_odt_intra'
            else:
                ValueError("No Partner Available")
        else:
            ValueError("No Partner Available")
        return self.env.ref(report_xml_id).report_action(self)

    # def invoice_odt_report_intra(self):
    #     self.ensure_one()
    #     if self.partner_id and self.env.company.state_id:
    #         if self.partner_id.state_id.id == self.env.company.state_id.id:
    ...
    #     else:
    #         self.so_type = ''
    #     return self.env.ref('cmr_customizations.text_tiles_invoice_report_odt_intra').report_action(self)
    ...
    def merge_credit_note_to_customer(self):
        for rec in self:
            if rec.journal_id.name == 'Credit Note Issue':
                customer = self.env['res.partner'].sudo().search([('name','=',rec.partner_id.name),('phone','=',rec.partner_id.phone)], limit=1)
                if customer:
                    customer.credit_note_ids.create({
                            'voucher_number': rec.name,
                            # 'pos_bill_number': picking.nhcl_pos_order.pos_reference,
                            'pos_bill_date': rec.invoice_date,
