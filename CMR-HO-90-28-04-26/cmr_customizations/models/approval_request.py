...
class ApprovalRequest(models.Model):
    ...
    _inherit = ['approval.request' ,'mail.thread.main.attachment', 'mail.activity.mixin', 'product.catalog.mixin']

    pi_type = fields.Selection(
        [('advertisement', 'Advertisement'), ('direct_po','Direct PO'), ('ho_operation', 'HO Operation'),('others', 'Others')],
    ...
    def action_create_purchase_orders(self):
        ...
        for line in self.product_line_ids:
            ...
            if line.unit_price:
                ...
                po_line_vals['default_rsp_margin'] = line.default_rsp_margin
                po_line_vals['rsp'] = line.pi_rsp_price
                po_line_vals['mrp'] = line.pi_mrp_price
    ...
    def _check_zone_consistency(self):
        for request in self:
            zones = request.product_line_ids.mapped('zone_id')
            # Filter out empty zones if any
            zones = [z.id for z in zones if z]
            if zones and len(set(zones)) > 1:
                raise ValidationError("All lines must have the same Zone.")


    def action_confirm(self):
        self.ensure_one()
        for rec in self:
            rec._check_zone_consistency()
            for line in rec.product_line_ids:
                ...
                if line.zone_id == False:
                    raise ValidationError("Zone should not be Empty.")
...
class ApprovalProductLine(models.Model):
    ...
    @api.depends('approval_request_id.product_line_ids')
    def _compute_s_no(self):
        for rec in self.approval_request_id:
            for index, line in enumerate(rec.product_line_ids, start=1):
                line.s_no = index
        ...
        for rec in self:
            if rec.product_id:
                ...
                if rec.nhcl_margin_rsp_type == 'rsp':
                    ...
                    # if rec.pi_rsp_price >= rec.pi_mrp_price:
                    #     raise ValidationError(
                    #         'RSP Price should not be greate then MRP Price')

    @api.onchange('pi_mrp_price')
    def onchange_mrp_price(self):
        for rec in self:
            if rec.pi_rsp_price > 0 and rec.pi_mrp_price and rec.pi_rsp_price >= rec.pi_mrp_price:
                raise ValidationError(
                    'RSP Price should not be greater then MRP Price')
    ...
    def _compute_product_domain(self):
        for record in self:
            ...
            for request in record.approval_request_id:
                zones = request.product_line_ids.mapped('zone_id')
                # Filter out empty zones if any
                zones = [z for z in zones if z]
                if zones and len(set(zones)) > 1:
                    # Group line S.No by zone
                    zone_map = {}
                    for line in request.product_line_ids:
                        zone_name = line.zone_id.name if line.zone_id else 'No Zone'
                        s_no = str(line.s_no or 'N/A')
                        zone_map.setdefault(zone_name, []).append(s_no)

                    # Build descriptive error message
                    zone_details = "\n".join(
                        [f"Zone '{zone}': Lines {', '.join(snos)}" for zone, snos in zone_map.items()]
                    )
                    raise ValidationError(
                        f"All lines must have the same Zone.\n"
                        f"The following line numbers belong to different zones:\n{zone_details}"
                    )
    ...
    def _get_purchase_order_values(self, vendor):
        ...
        vals = {
            ...
            'logistic_vendor': self.approval_request_id.logistic_vendor.id,
            'transporter': self.approval_request_id.transporter.id
        }
        return vals
    ...
    def write(self, vals):
        tracked_fields = [
            'nhcl_margin_rsp_type', 'unit_price', 'quantity',
            'default_rsp_margin', 'enter_rsp_margin', 'pi_rsp_price',
            'nhcl_taxes_id',]
        before = {
            rec.id: {
                field: rec[field] if field != 'nhcl_taxes_id' else rec[field].ids
        ...
        messages = {
            'nhcl_margin_rsp_type': "Margin type in line no %s changed from %s to %s",
            'unit_price': "Cost Price in line no %s changed from %s to %s",
            'quantity': "Quantity in line no %s changed from %s to %s",
            'default_rsp_margin': "Default RSP Margin in line no %s changed from %s to %s",
            'enter_rsp_margin': "Entered RSP Margin in line no %s changed from %s to %s",
            'pi_rsp_price': "RSP Price in line no %s changed from %s to %s",
            'nhcl_taxes_id': "Taxes in line no %s changed from %s to %s",
        }
        changed_fields = set(vals.keys()) & set(messages.keys())
        for line in self:
            old_vals = before[line.id]
            for field in changed_fields:
                if field == 'nhcl_taxes_id':
                    # Convert to IDs for comparison
                    old_ids = set(old_vals[field])
                    new_ids = set(line.nhcl_taxes_id.ids)
                    if old_ids != new_ids:
                        old_names = ', '.join(self.env['account.tax'].browse(old_ids).mapped('name')) or '—'
                        new_names = ', '.join(line.nhcl_taxes_id.mapped('name')) or '—'
                        line.approval_request_id.message_post(
                            body=_(messages[field]) % (line.s_no or '', old_names, new_names)
                        )
                else:
                    old = old_vals[field]
                    new = line[field]
                    if old != new:
                        line.approval_request_id.message_post(
                            body=_(messages[field]) % (line.s_no or '', old or '—', new or '—')
                        )
        return res
