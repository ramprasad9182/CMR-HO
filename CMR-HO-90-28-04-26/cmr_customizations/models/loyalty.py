...
class LoyaltyRule(models.Model):
    ...
    serial_ids = fields.Many2many('stock.lot', string="Serial No's", domain=[('product_id.item_type', '=', 'inventory')])
        ...
        for record in self:
            ...
            for full_value in df.iloc[:, 0].dropna():
                ...
                if "R" in val_str:
                    ...
                    extracted_serials.append(val_str)
            Lot = self.env['stock.lot'].sudo()
            # Base domain (common filters)
            base_domain = [
                ('product_qty', '>', 0),
                ('company_id.nhcl_company_bool', '=', False)]
            # Conditional domain
            if has_r_serials:
                domain = ['|', ('name', 'in', extracted_serials), ('ref', 'in', extracted_serials)] + base_domain
            else:
                domain = [('ref', 'in', extracted_serials)] + base_domain
            found_lots = Lot.search(domain)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('program_id'):
                raise UserError(_('Please save the Promotion before getting Serial Numbers.'))
...
class PosOrderLine(models.Model):
    ...
    amount_discount = fields.Float(string="Total Discount", store=True)
    nhcl_reward_id = fields.Char( string="NHCL Reward", store=True)