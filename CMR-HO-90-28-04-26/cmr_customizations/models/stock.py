...
class Picking(models.Model):
    ...
    has_landed_cost_status = fields.Boolean()

    stock_picking_delivery_ids = fields.One2many("stock.picking.barcode", "stock_picking_delivery_id")
    logistic_vendor = fields.Many2one('res.partner', string="Agent", copy=False,
                                      domain=[('group_contact.name', '=', 'Agent')])
    transporter = fields.Many2one('res.partner', string='Transporter', domain=[('group_contact.name', '=', 'Transporter')], copy=False, tracking=True)
    nhcl_bill_date = fields.Date(string="Bill Date")

    total_received_qty = fields.Float(
        string="Quantity",
        compute="_compute_received_totals"
    )
    ...
    nhcl_receipt_source = fields.Selection(
        [
            ('po', 'Against PO'),
            ('direct', 'Direct Receipt'),
            ('receipt_return', 'Against Receipt'),
        ],
        string="Receipt Type",
        compute="_compute_nhcl_receipt_source",
        store=True
    )
    fiscal_position_id = fields.Many2one(
        'account.fiscal.position',
        string="Trade Group Name",
        compute='_compute_fiscal_position',
        store=True
    )
    amount_discount = fields.Float(string="Total Discount", store=True)
    ...
    def _compute_fiscal_position(self):
        for rec in self:
            ...
            # From Sale Order
            if rec.sale_id and rec.sale_id.fiscal_position_id:
                fiscal_position = rec.sale_id.fiscal_position_id

            # From Purchase Order
            elif rec.purchase_id and rec.purchase_id.fiscal_position_id:
                fiscal_position = rec.purchase_id.fiscal_position_id
    ...
    def _compute_received_totals(self):
        for picking in self:
            ...
            for move in picking.move_ids_without_package:
                ...
                qty_total += received_qty
                untaxed_total += received_qty * price
            ...
            picking.total_received_qty = qty_total
            picking.total_untaxed_amount = untaxed_total
            picking.total_amount = total_with_tax
    ...
    #
    #                 # If tax is group (like 5% GST P)
    #                 if tax.amount_type == 'group':
    #                     for child_tax in tax.children_tax_ids:
    #                         name = child_tax.name.upper()
    #                         if "CGST" in name:
    #                             gst_set.add("CGST")
    #                         elif "SGST" in name:
    #                             gst_set.add("SGST")
    ...
    #
    #         if gst_set:
    #             picking.gst_types = "GST: " + ", ".join(sorted(gst_set))
    #         else:
    #             picking.gst_types = ""
    ...
    def _compute_nhcl_receipt_source(self):
        for picking in self:

            # Default
            picking.nhcl_receipt_source = False

            # 1️⃣ Goods Return → Against Receipt
            if picking.stock_picking_type == 'goods_return':
                picking.nhcl_receipt_source = 'receipt_return'
                continue

            # 2️⃣ Receipt + HO Operation
            if (
                    picking.stock_picking_type == 'receipt'
                    and picking.stock_type in ['ho_operation','direct_po','inter_state','intra_state']
            ):

                # Coming from PO
                if any(move.purchase_line_id for move in picking.move_ids_without_package):
                    picking.nhcl_receipt_source = 'po'  # Against PO
                else:
                    picking.nhcl_receipt_source = 'direct'  # Direct Receipt
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            vals['tracking_number'] = seq.next_by_code('stock.picking.tracking')
            vals['lr_number'] = seq.next_by_code('stock.picking.lr_number')

            if vals.get('return_id'):
                return_picking = self.env['stock.picking'].browse(vals['return_id'])
                vals['stock_bill_reference'] = return_picking.stock_bill_reference
    ...
    stock_type = fields.Selection(
        [('advertisement', 'Advertisement'), ('direct_po','Direct PO'),('ho_operation', 'HO Operation'), ('sub_contract', 'Sub Contracting'), ('data_import', 'Data Import'), ('inter_state', 'Inter State'),
    ...
    verification_success = fields.Selection([('matched', 'Matched')], string='Verification Status',
                                            tracking=True)
    stock_picking_type = fields.Selection(tracking=True, related='picking_type_id.stock_picking_type',store=True)
    ...
    picking_logistics_ids = fields.One2many('picking.logistic','picking_id')
    ...
    def _check_no_of_parcel(self):
        for rec in self:
            if rec.stock_picking_type == 'delivery' and rec.no_of_parcel <= 0 and rec.state in ('done'):
                raise ValidationError("No Of Parcels must be greater than 0 for Delivery.")
    ...
    def _get_last_serial_number(self):
        auto_generate_seq_rec = self.env['nhcl.master.sequence'].search(
            [('nhcl_code', '=', 'Auto Serial Number'), ('nhcl_state', '=', 'activate'),('nhcl_company_id','=',self.env.company.id)])
    ...
    def check_stock_verification_vs_move_lines(self):
        for picking in self:
            if not picking.stock_verification_ids or not picking.move_line_ids_without_package:
                continue

            # -----------------------------
            # MOVE LINE COLLECTION
            # -----------------------------
            unbrand_serials = set()
            brand_serial_counter = Counter()

            unbrand_lot_qty = defaultdict(float)
            brand_lot_qty = defaultdict(float)
            ...
            for line in picking.move_line_ids_without_package:
                ...
                if tracking == 'serial':
                    if line.type_product == 'un_brand' and line.lot_id:
                        unbrand_serials.add(line.lot_id.name)

                    elif line.type_product == 'brand' and line.internal_ref_lot:
                        brand_serial_counter[line.internal_ref_lot] += 1

                elif tracking == 'lot':
                    qty = line.quantity

                    if line.type_product == 'un_brand' and line.lot_id:
                        unbrand_lot_qty[line.lot_id.name] += qty

                    elif line.type_product == 'brand' and line.internal_ref_lot:
                        brand_lot_qty[line.internal_ref_lot] += qty

            # -----------------------------
            # VERIFICATION COLLECTION
            # -----------------------------
            unbrand_verified_serials = set()
            brand_verified_serial_counter = Counter()

            unbrand_verified_lot_qty = defaultdict(float)
            brand_verified_lot_qty = defaultdict(float)

            for v in picking.stock_verification_ids:
                if not v.stock_serial:
                    continue

                serial = v.stock_serial
                qty = v.stock_qty or 1

                # SERIAL TRACKING
                if serial in unbrand_serials:
                    unbrand_verified_serials.add(serial)

                elif serial in brand_serial_counter:
                    brand_verified_serial_counter[serial] += qty

                # LOT TRACKING
                elif serial in unbrand_lot_qty:
                    unbrand_verified_lot_qty[serial] += qty

                elif serial in brand_lot_qty:
                    brand_verified_lot_qty[serial] += qty

            # ===============================
            # ✅ SERIAL — UNBRAND
            # ===============================
            if unbrand_serials:
                if unbrand_serials != unbrand_verified_serials:
                    raise ValidationError(_(
                        "Unbranded serial mismatch:\n"
                        "Move Lines: %s\nVerified: %s"
                    ) % (
                                              ", ".join(sorted(unbrand_serials)),
                                              ", ".join(sorted(unbrand_verified_serials)),
                                          ))
            ...
            # ===============================
            serial_errors = []
            for serial, qty in brand_serial_counter.items():
                if brand_verified_serial_counter.get(serial, 0) != qty:
                    serial_errors.append(
                        f"{serial} → Move: {qty}, Verified: {brand_verified_serial_counter.get(serial, 0)}"
                    )

            if serial_errors:
                raise ValidationError(_(
                    "Branded serial count mismatch:\n%s"
                ) % "\n".join(serial_errors))
            ...
            # ===============================
            lot_errors = []
            for lot, qty in unbrand_lot_qty.items():
                if unbrand_verified_lot_qty.get(lot, 0) != qty:
                    lot_errors.append(
                        f"{lot} → Move: {qty}, Verified: {unbrand_verified_lot_qty.get(lot, 0)}"
                    )

            if lot_errors:
                raise ValidationError(_(
                    "Unbranded lot quantity mismatch:\n%s"
                ) % "\n".join(lot_errors))
            ...
            # ===============================
            brand_lot_errors = []
            for lot, qty in brand_lot_qty.items():
                if brand_verified_lot_qty.get(lot, 0) != qty:
                    brand_lot_errors.append(
                        f"{lot} → Move: {qty}, Verified: {brand_verified_lot_qty.get(lot, 0)}"
                    )

            if brand_lot_errors:
                raise ValidationError(_(
                    "Branded lot quantity mismatch:\n%s"
                ) % "\n".join(brand_lot_errors))


    def update_price_values(self):
        for pick in self.filtered(lambda p: p.stock_type == 'data_import'):
            # Map move_id → move_line records
            move_line_map = {}
            for line in pick.move_line_ids_without_package:
                move_line_map.setdefault(line.move_id.id, []).append(line)
            for move in pick.move_ids_without_package:
                lines = move_line_map.get(move.id, [])
                if lines:
                    vals = {
                        'cost_price': move.move_cp,
                        'mr_price': move.move_mrp,
                        'rs_price': move.move_rsp,
                        'internal_ref_lot': move.move_brand_barcode,
                    }
                    for line in lines:
                        line.write(vals)
    ...
    def button_validate(self):
        res = None
        for picking in self:
            if picking.quality_check_todo == True and picking.stock_type in ['ho_operation','direct_po']:
            ...
            if picking.stock_picking_type == 'receipt' and picking.stock_type == 'ho_operation':
                grc_qty = sum(picking.move_ids_without_package.mapped('quantity'))
                logis_qty = sum(picking.picking_logistics_ids.mapped('logistic_act_qty'))
                if grc_qty > logis_qty:
                    raise ValidationError("You cannot validate more than logistic actual qty.")
            if picking.picking_type_id.code == 'incoming' and (picking.stock_type in ['ho_operation','direct_po','sub_contract'] or is_subcontract):
                ...
                if move_lines:
                    raise UserError(_("Please assign a serial number for other product: %s") % move_lines[0].product_id.name)
                else:
                    unbranded_product_move_ids = picking.move_ids
                    un_move_lines = unbranded_product_move_ids.move_line_ids
                    no_serial_move_lines = un_move_lines.filtered_domain([('lot_id', '!=', False)])
                    if (not picking.is_confirm and un_move_lines) or (
                        ...
                        if res and picking.purchase_id and picking.is_landed_cost == 'no' and picking.stock_type in ['ho_operation','direct_po','sub_contract']:
                            for move_line in picking.move_ids_without_package.move_line_ids:
                                ...
                                if move_line.move_id.product_uom != purchase_line_id.product_uom:
                                    ...
                                    rs_price = purchase_line_id.rsp / move_line.move_id.product_uom.ratio
                                    mrp_price = purchase_line_id.mrp / move_line.move_id.product_uom.ratio

                                else:
                                    cost_price = purchase_line_id.price_unit

                                if move_line.product_id.id == purchase_line_id.product_id.id:
                                    rs_price = purchase_line_id.rsp
                                    mrp_price = purchase_line_id.mrp
                ...
                for res in picking:
                    ...
                    for move_line in res.move_line_ids:
                        corresponding_delivery_move_line = delivery.move_line_ids.filtered(
                            lambda dml: dml.product_id == move_line.product_id and dml.lot_id.name == move_line.lot_id.name
                        )
            ...
            if picking.is_landed_cost == 'no' and picking.stock_type in ['data_import']:
                picking.update_price_values()
                for move_line in picking.move_ids_without_package.move_line_ids:
                    move_line.lot_values_update()
                    lot_id = move_line.lot_id
                    cost_price = move_line.move_id.move_cp
                    mr_price = move_line.move_id.move_mrp
                    rs_price = move_line.move_id.move_rsp
                    lot_id.write({
                        'actual_cp': cost_price,
                        'cost_price': cost_price,
                        'mr_price': mr_price,
            ...
            if picking.stock_picking_type in ['return','damage'] and picking.company_id.nhcl_company_bool == False:
                if not picking.stock_verification_ids:
                    raise ValidationError("Verify Check is not done.")
                if picking.stock_verification_ids and picking.move_line_ids_without_package:
                    picking.check_stock_verification_vs_move_lines()
    ...
    def _onchange_verify_barcode(self):
        if self.verify_barcode:
            ...
            if re.match(gs1_pattern, barcode):
                ...
                matched_line = self.move_line_ids_without_package.filtered(
                    lambda x: x.lot_id.name == code and x.type_product == 'un_brand'
                )
                ...
                if not matched_line:
                    matched_line = self.move_line_ids_without_package.filtered(
                        lambda x: x.internal_ref_lot == code and x.type_product == 'brand'
                    )
                    if not matched_line:
                        raise ValidationError('No matching product or serial/lot number found.')
                    fallback = True

                product = matched_line[0].product_id
                ...
                if tracking_type == 'serial':
                    ...
                    if not fallback:
                        # TRUE SERIAL → UNIQUE → prevent duplicate
                        existing_line = self.stock_verification_ids.filtered(
                            lambda x: x.stock_serial == code
                        )
                        if existing_line:
                            raise ValidationError(f'Serial number {code} already scanned.')

                    # FALLBACK SERIAL → SAME BARCODE → ALWAYS NEW LINE
                    self.stock_verification_ids = [(0, 0, {
                        'stock_product_id': product_id,
                        'stock_serial': code,
                        'stock_qty': 1,
                ...
                # ================= LOT =================
                    ...
                    existing_product_line = self.stock_verification_ids.filtered(
                        lambda x: x.stock_product_id.id == product_id
                                  and x.stock_serial == code
                    )

                    if existing_product_line:
                        if existing_product_line.stock_qty + 1 > product_qty:
                            raise ValidationError(
                                f"Scanned quantity exceeds allowed quantity for product '{product.display_name}'."
                            )
                        existing_product_line.stock_qty += 1
    ...
    def reset_product_lines(self):
        for rec in self:
            ...
            raise ValidationError('You cannot delete the lines.')
...
class StockMove(models.Model):
    ...
    scan_count = fields.Integer(string="Scan Count", compute="get_scanned_count")

    @api.depends('lot_ids')
    def get_scanned_count(self):
        for move in self:
            if move.product_id.tracking == 'serial':
                move.scan_count = len(move.lot_ids)
            elif move.product_id.tracking == 'lot':
                move.scan_count = sum(move.move_line_ids.mapped('quantity'))
            else:
                move.scan_count = 0
    ...
    def _prepare_move_line_vals(self, *args, **kwargs):
        ...
        if self.picking_id.stock_type == 'data_import':
            vals.update({
                'internal_ref_lot': self.move_brand_barcode,
                'acutal_line_cp': self.move_cp,
                'cost_price': self.move_cp,
                'rs_price': self.move_mrp,
                'mr_price': self.move_rsp,
    ...
    def _get_new_picking_values(self):
        ...
        if res.get('origin'):
            ...
            partner = sale_id.partner_id.name or ''
            customer_id = partner.split('-')[-1] if '-' in partner else partner
    ...
    def action_assign_serial(self):
        if self.picking_id.picking_type_id.code == 'incoming' and self.picking_id.stock_type in ['ho_operation','direct_po','sub_contract','data_import']:
            raise UserError(
            ...
            if move.product_id.tracking in ['serial', 'lot'] and move.picking_id.picking_type_id.code == 'incoming':
                if float(move.quantity) == sum(
                    ...
                    if auto_generate:
                        ...
                        raise ValidationError(f"Auto Sequence Setup is not set for {self.env.company.name}")
    ...
    def write(self, vals):
        ...
        for move in self:
            if move.picking_id.stock_type in ['ho_operation','direct_po','sub_contract']:
            ...
            elif move.picking_id.stock_type in ['data_import']:
                if move.type_product == 'un_brand':
                    move.move_line_ids.filtered(lambda x: x.product_id == move.product_id).write(
                        {'type_product': move.type_product,
                         'internal_ref_lot': move.product_id.barcode})
...
class StockMoveLine(models.Model):
    ...
    approval_margin = fields.Integer(related='move_id.purchase_line_id.purchase_rsp_margin', string="App Margin",
                                     copy=False)
    segment = fields.Selection(string="segment", copy=False, store=True, related='product_id.segment')
    ...
    #             if move.picking_id.stock_type in ['ho_operation','direct_po','inter_state','intra_state']:
    ...
    #                     for serial in move_line.lot_ids:
    #                         if move.lot_id.name == serial.name:
    #                             move.type_product = serial.type_product
    #                             move.internal_ref_lot = serial.ref
    ...
    def lot_values_update(self):
        for move_line in self:
            if move_line.picking_id.picking_type_code == 'incoming' and move_line.picking_id.stock_type in ['ho_operation','direct_po','sub_contract','data_import']:
                move_line.lot_id.write({
                    'type_product': move_line.type_product,
    ...
    def get_from_info(self):
        for move_line in self:
            ...
            if picking and picking.stock_picking_type == 'delivery':
                value = "Main"
            elif picking and picking.stock_picking_type == 'receipt' and picking.picking_type_id.code == 'incoming':
                value = "Vendor"
            elif picking and picking.stock_picking_type == 'receipt' and picking.picking_type_id.code == 'outgoing':
                value = "Stock"
            elif picking and picking.stock_picking_type == 'manufacturing':
                value = "Production"
            elif picking and picking.picking_type_id.sequence_code == 'DSC':
                value = "Subcontractor"
            elif picking and picking.stock_picking_type == 'goods_return':
                value = "Stock"
...
class StockLot(models.Model):
    ...
    segment = fields.Selection( string="Segment", copy=False, related='product_id.segment')
...
class StockBackorderConfirmation(models.TransientModel):
    ...
    def process(self):
        for pick_id in self.pick_ids:
            if pick_id.picking_type_id.code == 'incoming' and pick_id.stock_type in ['ho_operation','direct_po','data_import'] and pick_id.is_confirm == False:
...
class StockLandedCost(models.Model):
    ...
    def button_validate(self):
        if not self.cost_lines:
            raise ValidationError(_("Please Add Transportation Charges in Landed Cost."))
        if not self.env.context.get('bypass_landed_wizard'):
            return {
                'name': _('Landed Cost Confirmation'),
                'type': 'ir.actions.act_window',
                'target': 'new',
                'res_model': 'nhcl.landed.cost.wizard',
                'view_mode': 'form',
                'view_id': self.env.ref(
                    'cmr_customizations.view_nhcl_landed_wizard_wizard'
                ).id,
                'context': {
                    'default_nhcl_landed_id': self.id,
                    'bypass_landed_wizard': True,
                },
            }
        ...
        for cost in self:
            for line in cost.valuation_adjustment_lines.filtered(lambda l: l.move_id):
                ...
                for move_line in line.move_id.move_line_ids:
                    if move_line.lot_id:
                        ...
                        if line.move_id.product_uom != purchase_line.product_uom:
                            cost_price = purchase_line.price_unit / line.move_id.product_uom.ratio
                        else:
                        ...
                        lot.write({
                            ...
                            'nhcl_margin_lot': move_line.approval_margin,
                            'category_8': purchase_line.purchase_category_id.id,
                            'picking_id': cost.picking_ids.id,
                        })
            ...
            cost.picking_ids.has_landed_cost_status = True
...
class StockVerification(models.Model):
    ...
    @api.model_create_multi
    def create(self, vals_list):
        # Step 1: collect product_ids
        product_ids = [
            vals.get('stock_product_id')
            for vals in vals_list
            if vals.get('stock_product_id')
        ]
        ...
        barcode_regex = re.compile(r'^\d{13}$')

        # Step 3: validation
        for vals in vals_list:
            product = product_map.get(vals.get('stock_product_id'))
            serial_or_barcode = vals.get('stock_serial')
            qty = vals.get('stock_qty', 0)

            if product and product.tracking == 'serial' and qty > 1:
                if not (serial_or_barcode and barcode_regex.match(serial_or_barcode)):
                    raise ValidationError(
                        _("You cannot add more than 1 quantity for a serial-tracked product (by serial number).")
                    )
...
class AdjustmentLines(models.Model):
    ...
    def compute_cost_price_per_unit(self):
        for rec in self:
            if rec.final_cost and rec.quantity and rec.cost_id.picking_ids.stock_type in ['ho_operation','direct_po','data_import']:
            ...
            elif rec.final_cost and rec.quantity and rec.cost_id.picking_ids.stock_type == 'sub_contract':
                pick = rec.cost_id.picking_ids
                for move in pick.move_ids_without_package:
                    if move.purchase_line_id and move.purchase_line_id.price_unit > 0.00 and move.product_id.id == rec.product_id.id:
                        former_cost = move.purchase_line_id.price_unit * rec.quantity
                        final_value = (former_cost + rec.additional_landed_cost )
                        temp_cost_price = round((final_value / rec.quantity),2)
                        rec.actual_cp_lc = round((former_cost / rec.quantity),2)
                        rec.nhcl_cost_price = temp_cost_price
            else:
...
class StockPickingBatch(models.Model):
    ...
    transfer_type = fields.Selection(
        ...
        string="Transfer Type",compute="_compute_transfer_type")
    ...
    vehicle_number = fields.Char(string='Vehicle Number')
    driver_name = fields.Char(string='Driver Name')
    ref_company_ids = fields.Many2many('res.company', string='Companies', compute='nhcl_get_companies', store=True)
    ...
    irn_number = fields.Char(related='invoice_ids.irn_number', copy=False, string="IRN Number")
    ewaybill_number = fields.Char(related='invoice_ids.ewaybill_number', copy=False, string="Ewaybill Number")
    invoice_dates_display = fields.Char(string="Invoice Date", compute="_compute_invoice_data")
    transfer_count = fields.Integer(string="No. of Packets",compute="_compute_transfer_count",store=True)
    ...
    def _compute_partners(self):
        for rec in self:
            rec.partner_ids = rec.picking_ids.mapped('partner_id')
    ...
    def action_view_nhcl_invoices(self):
        ...
        return {
            ...
            'context': {'default_move_type': 'out_invoice','create': False, 'delete': False, 'duplicate': False,'edit':False},
        }


    @api.onchange('company_id','user_id')
    @api.depends('company_id')
    def nhcl_get_companies(self):
        if self._context.get('default_transfer_type') in ['in']:
            cust = self.env['res.company'].search([('nhcl_company_bool', '=', False)])
            self.ref_company_ids = cust
        elif self._context.get('default_transfer_type') in ['out']:
            vend = self.env['res.company'].search([('nhcl_company_bool', '=', True)])
            self.ref_company_ids = vend
    ...
    def _compute_transfer_type(self):
        for batch in self:
            if batch.company_id and batch.company_id.nhcl_company_bool:
    ...
    def get_prices_and_attributes_ho(self):
        ...
        for rec in self:
            if rec.nhcl_fetch_values == False and rec.picking_type_id.stock_picking_type in ['return','goods_return','damage']:
                for picks in rec.picking_ids:
                    for move_line in picks.move_line_ids_without_package:
                        ...
                        ho_serial = self.env['stock.quant'].sudo().search([
                            ...
                            ('lot_id.company_id.nhcl_company_bool', '=', True)])
    ...
    def action_confirm(self):
        for batch in self:
            if batch.picking_type_id.stock_picking_type in ['return','goods_return'] and batch.nhcl_fetch_values == False:
                raise ValidationError("You should click get data")
            for picking in batch.picking_ids:
                if batch.transfer_type == 'out':
                    if picking.no_of_parcel <= 0:
...
class ManafacturingImportWizard(models.TransientModel):
    ...
    _description = "Manufacturing Import Wizard"
...
class PickingType(models.Model):
    ...
    stock_picking_type = fields.Selection(
        [('exchange', 'Customer-Return'), ('receipt', 'Receipt'), ('goods_return', 'Goods Return')
         ...
         ('return_main', 'Return-Main'),('manufacturing','Manufacturing'),('others','Others')], string='Type')
...
class StockVerificationImport(models.TransientModel):
    ...
    def action_import(self):
        ...
        barcode_row_map = {}
        # NEW LOGIC MAP (R barcode)
        r_barcode_qty_map = defaultdict(float)
        r_barcode_row_map = {}
        ...
        try:
            if self.file_type == 'excel':
                ...
                for idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
                    ...
                    # -----------------------------
                    # NEW LOGIC (R barcode only)
                    # -----------------------------
                    elif barcode:

                        # Case 1 → 13 length → digits only
                        if len(barcode) == 13:
                            barcode_qty_map[barcode] += qty
                            barcode_row_map[barcode] = idx

                        # Case 2 → not 13 → must have alphabet + digit
                        elif re.search(r'[A-Za-z]', barcode) and re.search(r'\d', barcode):
                            r_barcode_qty_map[barcode] += qty
                            r_barcode_row_map[barcode] = idx

                        # invalid → raise error
                        else:
                            raise UserError(
                                f"Row {idx}: Barcode '{barcode}' Not Found"
                            )


                    # elif barcode and barcode.startswith('R'):
                    #     r_barcode_qty_map[barcode] += qty
                    #     r_barcode_row_map[barcode] = idx
                    #
                    #
            ...
            # ====================================================
            # NEW R BARCODE PROCESSING
            # ====================================================

            for barcode, total_qty in r_barcode_qty_map.items():

                idx = r_barcode_row_map.get(barcode, 0)

                matching_lines = move_lines.filtered(
                    lambda ml: ml.internal_ref_lot == barcode and ml.type_product == 'brand'
                )

                if not matching_lines:
                    raise UserError(f"Row {idx}: Internal reference '{barcode}' not found in operations.")

                operation_qty = sum(matching_lines.mapped('quantity'))

                if total_qty > operation_qty:
                    raise UserError(
                        f"Row {idx}: Excel qty {total_qty} exceeds available qty {operation_qty} for '{barcode}'."
                    )

                product_line = matching_lines[0]
                tracking = product_line.product_id.tracking

                # SERIAL TRACKING + BRAND
                if tracking == 'serial':
...
class StockPickingBarcode(models.Model):
    ...
    @api.model_create_multi
    def create(self, vals_list):
        # Step 1: validate sequence config (once)
        delivery_check_seq = self.env['nhcl.master.sequence'].search(
            [('nhcl_code', '=', 'cmr.delivery'), ('nhcl_state', '=', 'activate')],
            limit=1
        )
        if not delivery_check_seq:
            raise ValidationError(
                _('The Delivery Check Sequence is not specified in the sequence master. Please configure it!')
            )

        # Step 2: collect picking_ids
        picking_ids = [
            vals.get('stock_picking_delivery_id')
            for vals in vals_list
            if vals.get('stock_picking_delivery_id')
        ]

        # Step 3: batch fetch pickings
        picking_map = {
            p.id: p for p in self.env['stock.picking'].browse(picking_ids)
        }

        # Step 4: process each record
        for vals in vals_list:
            picking = picking_map.get(vals.get('stock_picking_delivery_id'))
...
class StockLogisticEntry(models.Model):
    _name = 'picking.logistic'
    _description = "Picking Logistics"

    picking_id = fields.Many2one('stock.picking')
    picking_purchase = fields.Many2one('purchase.order', related='picking_id.purchase_id', store=True)
    logistic_id = fields.Many2one('logistic.screen.data', string="Logistic No")
    logistic_qty = fields.Float(string="Invoice Qty", related='logistic_id.invoice_quantity')
    logistic_act_qty = fields.Float(string="Actual Qty")
    ...
    @api.depends('logistic_id', 'logistic_id.invoice_quantity', 'logistic_act_qty')
    def _compute_remaining_qty(self):
        for record in self:
            if record.logistic_id:
                # Sum received quantity for all receipts of the same Logistic No
                total_received_qty = sum(
                    self.search([
                        ('logistic_id', '=', record.logistic_id.id)
                    ]).mapped('logistic_act_qty')
                )

                record.remaining_qty = (
                        (record.logistic_qty or 0.0) - total_received_qty
                )
            else:
                record.remaining_qty = 0.0

    @api.constrains('logistic_id', 'logistic_act_qty')
    def _check_logistic_qty_limit(self):
        """
        Prevent entering a quantity greater than the remaining logistic quantity
        across multiple receipt pickings.
        """
        for rec in self:
            if not (
                    rec.picking_id.purchase_id and
                    rec.picking_id.stock_type == 'ho_operation' and
                    rec.picking_id.stock_picking_type == 'receipt'
            ):
                continue

            if not rec.logistic_id:
                continue

            if rec.logistic_act_qty <= 0:
                raise ValidationError("Quantity must be greater than zero.")

            # Calculate total already received (excluding current record)
            total_received_qty = sum(
                self.search([
                    ('logistic_id', '=', rec.logistic_id.id),
                    ('id', '!=', rec.id)
                ]).mapped('logistic_act_qty')
            )

            remaining_qty = (rec.logistic_qty or 0.0) - total_received_qty

            if rec.logistic_act_qty > remaining_qty:
                raise ValidationError(
                    f"You can only enter up to the remaining quantity.\n\n"
                    f"Invoice Qty     : {rec.logistic_qty}\n"
                    f"Already Received: {total_received_qty}\n"
                    f"Remaining Qty   : {remaining_qty}\n"
                    f"Entered Qty     : {rec.logistic_act_qty}"
                )
