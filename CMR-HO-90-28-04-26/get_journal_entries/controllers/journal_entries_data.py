...
class GETJournals(http.Controller):
    ...
    def customer_invoice_journal_items(self, **kwargs):
        try:
            ...
            for line in lines:
                ...
                account_group = line.account_id.group_id.name or ''
                account_sequence = line.account_id.group_id.sequence or ''
                ...
                acc_key = (account, account_code)

                if acc_key not in grouped[header_key]['Lines']:
                    grouped[header_key]['Lines'][acc_key] = {
                        'Account': account,
                ...
                grouped[header_key]['Lines'][acc_key]['Debit'] += line.debit or 0.0
                grouped[header_key]['Lines'][acc_key]['Credit'] += line.credit or 0.0
            ...
            for header in grouped.values():
                ...
                # Optional sorting (by sequence, then code)
                sorted_lines = sorted(
                    header['Lines'].values(),
                    key=lambda x: (
                        str(x.get('Sequence') or ''),
                        str(x.get('AccountCode') or '')
                    )
                )

                for acc in sorted_lines:
                    lines_list.append({
                        'Account': acc['Account'],
    ...
    @http.route('/odoo/get_customer_payment_against_journal_items', type='http', auth='public', methods=['GET'],
                csrf=False)
    def get_customer_payment_against_journal_items(self, **kwargs):
        try:
            integration = request.env['tally.integration'].sudo().search(
            ...
            domain = [
                ('move_id.state', '=', 'posted'),
                ('move_id.journal_id.name', 'in', payment_journals),
                ('move_id.nhcl_flag', '=', 'n'),
            ]
            ...
            for header in grouped.values():
                ...
                # Sort by Sequence then AccountCode
                sorted_lines = sorted(
                    header['Lines'].values(),
                    key=lambda x: (
                        str(x.get('Sequence') or ''),
