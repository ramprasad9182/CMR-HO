...
class HoAuditReport(models.Model):
    ...
    _description = "HO Audit Report"
    _rec_name = 'name'

    from_date = fields.Datetime('From Date')
    to_date = fields.Datetime('To Date')
    nhcl_store_id = fields.Many2many('nhcl.ho.store.master', string='Company')
    plan_id = fields.Many2one('stock.inventory', string='Audit Plan', domain=[('state', '=', 'done'), ('company_id.nhcl_company_bool', '=', True)])
    ...
    name = fields.Char(string='Name', default='HO POS Audit Report')
    ...
    def action_ho_pos_audit_report_line(self):
        self.ensure_one()

        return {
            'type': 'ir.actions.act_window',
            'name': 'HO POS Audit Report',
            'res_model': 'ho.audit.report.line',
            'view_mode': 'tree,pivot',
            'domain': [('ho_audit_report_id', '=', self.id)],
            'context': {
                'default_ho_audit_report_id': self.id
            }
        }
...
class HoAuditReportLine(models.Model):
    ...
    _description = "HO Audit Report Line"
