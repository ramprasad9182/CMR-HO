...
class NhclPOSHourReport(models.Model):
    ...
    #                         has_pack_lot = bool(data.get("pack_lot_ids"))
    #                         exit_line = self.nhcl_pos_hour_report_ids.filtered(
    #                             lambda x: x.nhcl_order_ref == data.get("order_id")[0]["name"] and has_pack_lot)
    #                         print("sdfguihaflk",data)
    #                         if exit_line:
    ...
    def get_pos_order_hour_report(self):
        ...
        try:
            for store in self.nhcl_store_id:
                ...
                # Convert from_date and to_date to local timezone
                from_date = datetime.strftime(
                    pytz.utc.localize(
                        datetime.strptime(str(self.from_date), DEFAULT_SERVER_DATETIME_FORMAT)
                    ).astimezone(local_tz),
                    "%Y-%m-%d %H:%M:%S"
                )
                ...
                to_date = datetime.strftime(
                    pytz.utc.localize(
                        datetime.strptime(str(self.to_date), DEFAULT_SERVER_DATETIME_FORMAT)
                    ).astimezone(local_tz),
                    "%Y-%m-%d %H:%M:%S"
                )
                ...
                # Find store master
                store_master = self.env['nhcl.ho.store.master'].search(
                    [('id', '=', store.id)],
                    limit=1
                )
                ...
                for data in pos_data_line:
                    ...
                    # Parse datetime (with / without microseconds)
                    try:
                        date_order = datetime.strptime(
                            date_order_str, "%Y-%m-%dT%H:%M:%S.%f"
                        )
                    except ValueError:
                        date_order = datetime.strptime(
                            date_order_str, "%Y-%m-%dT%H:%M:%S"
                        )

                    date_order_local = datetime.strftime(
                        pytz.utc.localize(date_order).astimezone(local_tz),
                        "%Y-%m-%d %H:%M:%S"
                    )
                    ...
                    # Order reference
                    order_ref = data.get("order_id", [{}])[0].get("name")
                    if not order_ref:
                        continue

                    qty = data.get("qty", 0)
                    subtotal = data.get("price_subtotal", 0.0)
                    tax_inc_total = data.get("price_subtotal_incl", 0.0)
                    is_reward = data.get("is_reward_line", False)
                ...
                pos_config = self.env['pos.config'].search(
                    [('company_id.name', '=', store_master.nhcl_store_name.name)],
                    limit=1
                )

                for order_ref, values in grouped_data.items():
                    receipt_no = self.env['pos.order'].search(
                        [('company_id.name', '=', store_master.nhcl_store_name.name), ('name', '=', order_ref)],
                        limit=1
                    )
                    vals_list.append({
                        'nhcl_order_ref': order_ref,
                        'nhcl_bill_receipt_no': receipt_no.pos_reference,
            ...
            print(f"Failed to retrieve POS order data: {e}")
