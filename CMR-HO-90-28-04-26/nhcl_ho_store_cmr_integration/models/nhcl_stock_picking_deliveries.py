...
class Picking(models.Model):
    ...
    def auto_validate_pos_exchange_orders(self):
        ho_store_id = self.env['nhcl.ho.store.master'].sudo().search(
            [('nhcl_store_type', '=', 'ho'), ('nhcl_active', '=', True), ])

        store_customer_return_orders = self.env['stock.picking'].sudo().search(
            [('nhcl_store_delivery', '=', True), ('state', 'not in', ['done', 'cancel']),
    ...
    nhcl_receipt_status = fields.Boolean(string="Status")
    ...
    #         [('nhcl_store_type', '!=', 'ho'),
    #          ('nhcl_active', '=', True),
    #          ('nhcl_store_name.company_id.name', '=', self.company_id.name)],
    #         limit=1
    #     )
    #     if not ho_store_id:
    #         return
    ...
    #     })
    #
    #     _logger.info(f"Start Receipt Integration: {self.name}")
    #
    #     # ---------------- LOCATION ----------------
    #     try:
    #         location = session.get(
    #             f"{base_url}/stock.location/search?domain={[('usage', '=', 'supplier')]}"
    #         ).json()
    #
    #         # partner_suffix = self.partner_id.name.split('-')[-1]
    #
    #         location_dest = session.get(
    #             f"{base_url}/stock.location/search?domain={[('name', '=', self.location_dest_id.name)]}"
    #         ).json()
    #
    ...
    #
    #     except Exception:
    #         msg = f"{self.name} - Location not found"
    #         _logger.error(msg)
    #
    #         ho_store_id.create_cmr_transaction_server_replication_log('failure', msg)
    #         ho_store_id.create_cmr_transaction_replication_log(
    #             'stock.picking', self.id, 200, 'add', 'failure', msg
    #         )
    #         return
    #
    #     # ---------------- PICKING TYPE ----------------
    #     try:
    #         picking_type_data = session.get(
    #             f"{base_url}/stock.picking.type/search?domain={[('stock_picking_type', '=', 'receipt')]}"
    #         ).json()
    #
    #         stock_picking_type = picking_type_data.get("data")[0]["id"]
    #
    #     except Exception:
    #         msg = f"{self.name} - Picking type not found"
    #         _logger.error(msg)
    #
    #         ho_store_id.create_cmr_transaction_server_replication_log('failure', msg)
    #         ho_store_id.create_cmr_transaction_replication_log(
    ...
    #
    #     # ---------------- STOCK LINES ----------------
    #     stock_detail_lines, stock_actual_lines, missing_products = [], [], []
    #     merged_actual = {}
    #
    #     for line in self.move_line_ids:
    #         product = line.product_id
    #
    #         nhcl_id = str(product.nhcl_id or '').strip()
    #
    #         if not nhcl_id:
    #             raise ValidationError(f"NHCL ID missing for {product.name}")
    #
    #         # ---------------- PRODUCT SEARCH (DIRECT) ----------------
    #         _logger.info(f"Searching product for NHCL ID: {nhcl_id}")
    #
    #         product_data = session.get(
    #             f"{base_url}/product.product/search?domain={[('nhcl_id', '=', nhcl_id)]}"
    #         ).json()
    #
    #         if not product_data.get("data"):
    #             _logger.warning(f"Product not found in store for NHCL ID: {nhcl_id}")
    #             missing_products.append(product.display_name)
    #             continue
    #
    #         product_id = product_data["data"][0]["id"]
    #
    #         _logger.info(f"Mapped product: {product.name} -> {product_id}")
    #
    #         # ---------------- COMMON VALUES ----------------
    #         mr_price = line.mr_price or 0.0
    #         stock_serial = line.lot_id.name if line.lot_id else False
    #
    #         # ---------------- DETAIL LINE ----------------
    #         stock_detail_lines.append((0, 0, {
    ...
    #             'lot_name': stock_serial,
    #             'internal_ref_lot': line.internal_ref_lot or False,
    #             'nhcl_purchase_indent_reference': self.sale_id.nhcl_indent_id.partner_ref,
    #         }))
    #
    #         # ---------------- ACTUAL LINE ----------------
    #         actual_line = {
    #             'stock_product_id': product_id,
    #             'stock_product_barcode': line.internal_ref_lot,
    #             'mr_price': mr_price,
    #             'rs_price': line.rs_price or 0,
    #             'cost_price': line.cost_price or 0,
    #             'type_product': line.type_product,
    ...
    #         }
    #
    #         key = (product_id, stock_serial)
    #
    #         if key in merged_actual:
    #             merged_actual[key]['stock_qty'] += line.quantity
    #         else:
    #             merged_actual[key] = actual_line
    #
    #     for data in merged_actual.values():
    #         stock_actual_lines.append((0, 0, data))
    #
    #     # ---------------- VALIDATION ----------------
    #     if missing_products:
    #         msg = f"Missing products: {', '.join(missing_products)}"
    #         _logger.error(msg)
    #
    #         ho_store_id.create_cmr_transaction_server_replication_log('failure', msg)
    #         ho_store_id.create_cmr_transaction_replication_log(
    ...
    #         'lr_number': self.lr_number or None,
    #         'vehicle_number': self.vehicle_number or None,
    #         'driver_name': self.driver_name or None,
    #         'no_of_parcel': self.no_of_parcel or None,
    #         'nhcl_tracking_number': self.tracking_number,
    #         'location_id': location_id,
    #         'location_dest_id': location_dest_id,
    #         'move_line_ids_without_package': stock_detail_lines,
    #         # 'stock_verification_ids': stock_actual_lines,
    #     }
    #
    #     # ---------------- API CALL ----------------
    #     try:
    #         check = session.get(
    #             f"{base_url}/stock.picking/search?domain={[('origin', '=', self.name)]}"
    #         ).json()
    #
    ...
    #
    #                 ho_store_id.create_cmr_transaction_server_replication_log('failure', message)
    #                 ho_store_id.create_cmr_transaction_replication_log(
    #                     'stock.picking', self.id, 200, 'add', 'failure', message
    #                 )
    #             else:
    #                 self.sudo().nhcl_receipt_status = True
    ...
    #                 ho_store_id.create_cmr_transaction_replication_log(
    #                     'stock.picking', self.id, 200, 'add', 'success', message
    #                 )
    #
    #     except Exception as e:
    #         _logger.error(str(e))
    #
    #         ho_store_id.create_cmr_transaction_server_replication_log('failure', str(e))
    #         ho_store_id.create_cmr_transaction_replication_log(
    #             'stock.picking', self.id, 500, 'add', 'failure', str(e)
    #         )
    ...
    #         stock_picking_data_domain = [('origin', '=', self.name)]
    #         stock_picking_url = f"http://{store_ip}:{store_port}/api/stock.picking/search"
    #         stock_picking_search_url = f"{stock_picking_url}?domain={stock_picking_data_domain}"
    ...
    #                 # missing_products.append(move_line.product_id.display_name)
    #                 continue
    ...
    #                 actual_line = {
    #                     'stock_product_id': product_id,
    #                     'stock_product_barcode': move_line.internal_ref_lot,
    #                     'mr_price': mr_price,
    #                     'rs_price': move_line.rs_price if move_line.rs_price else 0,
    #                     'cost_price': move_line.cost_price if move_line.cost_price else 0,
    #                     'type_product': move_line.type_product,
    #                     'stock_serial': move_line.lot_id.name,
    #                     'stock_qty': move_line.quantity,
    #                     'segment': move_line.segment,
    #                     'categ_1': product_categ_1_id if move_line.categ_1 else False,
    #                     'categ_2': product_categ_2_id if move_line.categ_2 else False,
    #                     'categ_3': product_categ_3_id if move_line.categ_3 else False,
    ...
    #                     'descrip_6': product_descrip_6_id if move_line.descrip_6 else False,
    #                 }
    #                 stock_actual_lines.append((0, 0, actual_line))
    #         # if missing_products:
    #         #     raise ValidationError(
    #         #         "The following products are missing in store:\n"
    #         #         + "\n".join(missing_products) + "\nIntegrate those products first."
    #         #     )
    #
    ...
    #             try:
    #                 response = requests.get(stock_picking_search_url, headers=headers_source)
    #                 response.raise_for_status()  # Raises an HTTPError for bad responses
    #
    #                 # Parse the JSON response
    #                 data = response.json()  # Now `data` is a dictionary
    #                 picking_data = data.get("data", [])
    #                 if not picking_data:
    ...
    #                         ho_store_id.create_cmr_transaction_server_replication_log('success', message)
    #                         ho_store_id.create_cmr_transaction_replication_log(response_json['object_name'], self.id,
    #                                                                            200,
    #                                                                            'add', 'failure', message)
    #                     else:
    ...
    def get_receipts_data(self):
        ...
        base_url = f"http://{ho_store.nhcl_terminal_ip}:{ho_store.nhcl_port_no}/api"
        session.headers.update({
            'api-key': ho_store.nhcl_api_key,
            'Content-Type': 'application/json'
        })
        ...
        try:
            location_data = session.get(
                f"{base_url}/stock.location/search?domain={[('usage', '=', 'supplier')]}&fields=['name','id']", timeout=10
            ).json()
            dest_data = session.get(
                f"{base_url}/stock.location/search?domain={[('name', '=', self.location_dest_id.name)]}&fields=['name','id']", timeout=10
            ).json()

            location_id = location_data.get("data")[0]["id"]
            location_dest_id = dest_data.get("data")[0]["id"]
        except Exception:
            msg = f"{self.name} - Location not found"
            _logger.error(msg)
            ho_store.create_cmr_transaction_server_replication_log('failure', msg)
        ...
        try:
            picking_type_data = session.get(
                f"{base_url}/stock.picking.type/search?domain={[('stock_picking_type', '=', 'receipt')]}", timeout=10
            ).json()
            stock_picking_type = picking_type_data.get("data")[0]["id"]
        except Exception:
            msg = f"{self.name} - Picking type not found"
        ...
        # ---------------- BULK PRODUCT FETCH ----------------
        nhcl_ids = list(
            {str(line.product_id.nhcl_id).strip() for line in self.move_line_ids if line.product_id.nhcl_id})
        if not nhcl_ids:
            raise ValidationError("No NHCL IDs found for products in this receipt.")

        try:
            product_response = session.get(
                f"{base_url}/product.product/search?domain={[('nhcl_id', 'in', nhcl_ids)]}", timeout=20
            ).json()
            product_map = {str(p['nhcl_id']).strip(): p['id'] for p in product_response.get("data", [])}
        except Exception:
            msg = f"{self.name} - Failed to fetch products from store"
            _logger.error(msg)
            ho_store.create_cmr_transaction_server_replication_log('failure', msg)
            raise ValidationError(msg)
        ...
        sale_ref = self.sale_id.nhcl_indent_id.partner_ref if self.sale_id else None

        for line in self.move_line_ids:
            nhcl_id = str(line.product_id.nhcl_id or '').strip()
            if not nhcl_id:
                raise ValidationError(f"NHCL ID missing for {line.product_id.name}")

            product_id = product_map.get(nhcl_id)
            if not product_id:
            ...
            stock_detail_lines.append((0, 0, {
                ...
                'nhcl_purchase_indent_reference': sale_ref,
            }))
        ...
        try:
            check = session.get(
                f"{base_url}/stock.picking/search?domain={[('origin', '=', self.name)]}", timeout=20
            ).json()
            ...
            if not check.get("data"):
                response = session.post(
                    f"{base_url}/stock.picking/create",
                    json=[stock_picking_data],
                    timeout=300  # increased timeout for large payload
                ).json()
                ...
                if not success:
                    _logger.error(message)
                    ho_store.create_cmr_transaction_server_replication_log('failure', message)
                    ho_store.create_cmr_transaction_replication_log(
                        'stock.picking', self.id, 200, 'add', 'failure', message
                    )
                else:
                    self.sudo().nhcl_receipt_status = True
                    _logger.info(message)
                    ho_store.create_cmr_transaction_server_replication_log('success', message)
                    ho_store.create_cmr_transaction_replication_log(
                        'stock.picking', self.id, 200, 'add', 'success', message
                    )
            ...
            _logger.error(str(e))
            ho_store.create_cmr_transaction_server_replication_log('failure', str(e))
            ho_store.create_cmr_transaction_replication_log(
                'stock.picking', self.id, 500, 'add', 'failure', str(e)
            )
    ...
    def get_delivery_orders(self):
        ...
        store_port = ho_store_id.nhcl_port_no
        store_api_key = ho_store_id.nhcl_api_key
        session.headers.update({'api-key': store_api_key, 'Content-Type': 'application/json'})

        # --- Location Fetch ---
        location_url = f"http://{store_ip}:{store_port}/api/stock.location/search"
        location = session.get(f"{location_url}?domain={[('usage', '=', 'supplier')]}&fields=['name','id']").json()
        partner_suffix = self.partner_id.name.split('-')[-1]
        location_dest = session.get(f"{location_url}?domain={[('name', '=', partner_suffix)]}&fields=['name','id']").json()

        try:
            location_id = location.get("data")[0]["id"]
            location_dest_id = location_dest.get("data")[0]["id"]
        except Exception:
            _logger.error(f"Location not found for partner {self.partner_id.name}")
            ho_store_id.create_cmr_transaction_replication_log(
                'stock.picking', self.id, 200, 'add', 'failure',
                f"{self.name}, {self.dest_location_id.name} Destnation Location Not found"
            )
            return

        # --- Picking Type ---
        picking_type_url = f"http://{store_ip}:{store_port}/api/stock.picking.type/search"
        picking_type_data = session.get(f"{picking_type_url}?domain={[('stock_picking_type', '=', 'receipt')]}").json()
        try:
            ...
            ho_store_id.create_cmr_transaction_replication_log(
                'stock.picking', self.id, 200, 'add', 'failure',
                f"{self.name}, {self.picking_type_id.name} Picking Type Not found"
            )
            return

        # --- Transporter ---
        transporter_id = False
        if self.transpoter_id:
            transporter_url = f"http://{store_ip}:{store_port}/api/dev.transport.details/search"
            transporter_data = session.get(
                f"{transporter_url}?domain={[('nhcl_id', '=', self.transpoter_id.nhcl_id)]}").json()
            transporter_ids = transporter_data.get("data")
            transporter_id = transporter_ids[0]["id"] if transporter_ids else False
        ...
        # --- Transporter Route ---
        transporter_route_id = False
        if self.transpoter_route_id:
            route_url = f"http://{store_ip}:{store_port}/api/dev.routes.details/search"
            route_data = session.get(
                f"{route_url}?domain={[('nhcl_id', '=', self.transpoter_route_id.nhcl_id)]}").json()
            route_ids = route_data.get("data")
            transporter_route_id = route_ids[0]["id"] if route_ids else False
            if not transporter_route_id:
                ho_store_id.create_cmr_transaction_replication_log(
                    'stock.picking', self.id, 200, 'add', 'failure',
                    f"{self.name}, {self.transpoter_route_id.name} Transporter Route Not found"
                )

        # --- ✅ Batch Product Fetch ---
        barcodes = [m.product_id.barcode for m in self.move_ids if m.product_id.barcode]
        product_map = {}
        if barcodes:
            product_url = f"http://{store_ip}:{store_port}/api/product.product/search"
            product_data = session.get(f"{product_url}?domain={[('barcode', 'in', barcodes)]}").json()
            for prod in product_data.get("data", []):
                product_map[prod['barcode']] = prod

        # --- Prepare Stock Lines ---
        stock_detail_lines, stock_actual_lines, missing_products = [], [], []

        for move in self.move_ids:
            product_info = product_map.get(move.product_id.barcode)
            if not product_info:
            ...
            merged_actual = {}  # key = stock_serial (lot number)

            for line in move.move_line_ids:
                mr_price = line.mr_price or 0.0
                stock_serial = line.lot_id.name

                # -------------------------
                # Stock Detail Line (no merging)
                # -------------------------
                detail_line = {
                    'product_id': product_id,
                    'mr_price': mr_price,
                ...
                actual_line = {
                    ...
                    # Category fields
                    'nhcl_categ_1': line.categ_1.name if line.categ_1 else None,
                    'nhcl_categ_2': line.categ_2.name if line.categ_2 else None,
                    'nhcl_categ_3': line.categ_3.name if line.categ_3 else None,
                    'nhcl_categ_4': line.categ_4.name if line.categ_4 else None,
                    'nhcl_categ_5': line.categ_5.name if line.categ_5 else None,
                    'nhcl_categ_6': line.categ_6.name if line.categ_6 else None,
                    'nhcl_categ_7': line.categ_7.name if line.categ_7 else None,
                    'nhcl_categ_8': line.categ_8.name if line.categ_8 else None,

                    # Description fields
                    'nhcl_descrip_1': line.descrip_1.name if line.descrip_1 else None,
                    'nhcl_descrip_2': line.descrip_2.name if line.descrip_2 else None,
                    'nhcl_descrip_3': line.descrip_3.name if line.descrip_3 else None,
                    'nhcl_descrip_4': line.descrip_4.name if line.descrip_4 else None,
                    'nhcl_descrip_5': line.descrip_5.name if line.descrip_5 else None,
                    'nhcl_descrip_6': line.descrip_6.name if line.descrip_6 else None,
                }

                # -------------------------
                # Merging Logic
                # -------------------------
                if stock_serial in merged_actual:
                    # update qty only
                    merged_actual[stock_serial]['stock_qty'] += line.quantity
                else:
                    merged_actual[stock_serial] = actual_line

            # ----------------------------------------
            # Convert final merged dict into Odoo format
            # ----------------------------------------
            for data in merged_actual.values():
                stock_actual_lines.append((0, 0, data))

        barcode_lines=[]
        for rec in self.stock_picking_delivery_ids:
            barcode_data = {
        ...
        if missing_products:
            product_names = ', '.join(missing_products)
            raise ValidationError(
                f"The following products are missing in store '{ho_store_id.nhcl_store_name.name}':\n{product_names}"
            )
        ...
        stock_picking_data = {
            ...
            'no_of_parcel': self.no_of_parcel or None,
            'nhcl_tracking_number': self.tracking_number,
            'transpoter_id': transporter_id,
            'transpoter_route_id': transporter_route_id,
            'location_id': location_id,
            'location_dest_id': location_dest_id,
            'move_line_ids_without_package': stock_detail_lines,
            'stock_verification_ids': stock_actual_lines,
        ...
        try:
            response = session.get(f"{stock_picking_url}?domain={[('origin', '=', self.name)]}")
            response.raise_for_status()
            picking_data = response.json().get("data", [])

            if not picking_data:
                stores_data = session.post(store_url_data, json=[stock_picking_data])
                stores_data.raise_for_status()
                response_json = stores_data.json()
                ...
                if not response_json.get("success", True):
                    ...
                    ho_store_id.create_cmr_transaction_server_replication_log('failure', message)
                    ho_store_id.create_cmr_transaction_replication_log(
                        response_json.get('object_name', 'stock.picking'),
                        self.id, 200, 'add', 'failure', message
                    )
                else:
                    self.sudo().nhcl_delivery_status = True
                    self.is_replicated = True
                    _logger.info(f"Successfully created stock picking: {message} from {store_ip}")
                    ho_store_id.sudo().create_cmr_transaction_server_replication_log('success', message)
                    ho_store_id.sudo().create_cmr_transaction_replication_log(
                        response_json.get('object_name', 'stock.picking'),
            ...
            _logger.error(
                f"Failed to create stock picking for '{self.name}' with partner '{self.partner_id.name}'. Error: {e}")
            self.sudo().nhcl_delivery_status = False
            ho_store_id.sudo().create_cmr_transaction_server_replication_log('failure', str(e))
            ...
            try:
                ...
                for line in each.picking_ids:
                    ...
                    if store_batch_data.get('data') and batch_id:
                        ...
                        stock_picking_domain = [('stock_picking_type', '=', 'receipt')]
                ...
                if not store_batch_data.get('data'):
                    ...
                    if not response_json.get("success", True):
                        ...
                        # created_picking_ids = response_json.get("create_id")
                        # for picking_id in created_picking_ids:
                        #     update_batch_url = f"http://{store_ip}:{store_port}/api/stock.picking/{picking_id}"
                        #     update_picking_data = {
                        #         'nhcl_batch_number': each.name  # HO batch number
                        #     }
                        #     requests.put(update_batch_url, headers=headers_source, json=update_picking_data)
