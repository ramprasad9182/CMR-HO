...
class StoresWiseDivisionReplicationLog(models.Model):
    _name = 'store.wise.division.replication.log'
    _description = "Store Wise Division Replication Log"

    nhcl_serial_no = fields.Char('S.No',default=lambda self: self.env['ir.sequence'].next_by_code('store.wise.division.replication.log'),readonly=True,copy=False)
