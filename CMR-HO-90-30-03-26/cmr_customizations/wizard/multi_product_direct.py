        ...
        if self.brand_serials:
            ...
            for serials in self.brand_serials:
                ...
                new_lines.append((0, 0, {
                    ...
                    'lot_cp': serials.actual_cp,
                    'lot_mrp': serials.mr_price,
                    'lot_rsp': serials.rs_price,
                    'quantity': qty,
                    'tracking': serials.product_id.tracking,
                    'nhcl_grc_bill_number': serials.picking_id.stock_bill_reference,
                    'nhcl_grc_bill_date': serials.picking_id.nhcl_bill_date,
                }))
...
class MultiPurchaseProductWizardDirect(models.TransientModel):
    _name = 'multi.purchase.product.wizard.direct'
    _description = 'Wizard to select multiple products'
    ...
    def action_add_products(self):
        ...
        for product in self.product_ids:
            # Check if product already exists in the request's lines
            existing_line = purchase_line.search([
