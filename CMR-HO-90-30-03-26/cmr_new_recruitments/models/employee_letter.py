...
class EmployeeLetter(models.Model):
    ...
    pt_m = fields.Float(string="PT (Per Month)", compute="_compute_pt_m",
                        store=True)
    ...
    @api.depends('ctc_m', 'employee_id.company_id.state_id')
    def _compute_pt_m(self):
        TaxMaster = self.env['state.tax.master']

        for rec in self:
            pt = 0.0
            ctc = rec.ctc_m or 0.0
            state = rec.employee_id.company_id.state_id

            if ctc and state:
                tax_master = TaxMaster.search(
                    [('state_id', '=', state.id)],
                    limit=1
                )

                if tax_master:
                    slab = tax_master.slab_ids.filtered(
                        lambda s: s.ctc_from <= ctc <= s.ctc_to
                    )
                    if slab:
                        pt = slab[0].tax_amount
    ...
    #
    #         # ✅ ESIC calculation only if CTC <= 21000
    ...
    #         record.basic_m = math.ceil(record.ctc_m * 0.60)
    #         record.hra_m = math.ceil(record.ctc_m * 0.30)
    #         # ✅ Bonus logic based on ctc_type
    #         if record.ctc_type == 'with_bonus':
    #             record.bonus_m = math.ceil(record.ctc_m * 0.10)
    #         else:
    #             record.bonus_m = 0
    #         record.other_allowance_m = math.ceil(record.ctc_m * 0.10)
    ...
    #
    #         # ✔ Employer PF logic
    #         if record.ctc_type != 'non_ctc':
    #             employer_pf = math.ceil(record.basic_m * 0.12)
    #             if employer_pf > 1800:
    #                 employer_pf = 1800
    #             record.employer_pf_m = employer_pf
    #         else:
    #             record.employer_pf_m = 0
    #
    #         if record.ctc_m < 15000:
    #             record.pt_m = 0
    #         elif record.ctc_m <= 20000:
    ...
    #         record.net_take_home_m = math.ceil(
    #             record.basic_m + record.hra_m + record.other_allowance_m
    #             - record.pf_m - record.pt_m - record.esic_m - record.employer_pf_m
    #         )
    #
    # # Annual salary calculation
    # @api.depends('ctc_m', 'basic_m', 'hra_m', 'other_allowance_m', 'ctc_type')
    ...
    #         record.pf = math.ceil(record.pf_m * 12)
    #
    #         # ✔ Employer PF annual logic
    #         if record.ctc_type != 'non_ctc':
    #             record.employer_pf = math.ceil(record.pf_m * 12)
    #         else:
    #             record.employer_pf = 0
    #
    #         record.pt = math.ceil(record.pt_m * 12)
    #
    #         # ✔ Bonus logic based on ctc_type
    ...
    #         record.net_take_home = math.ceil(
    #             record.basic + record.hra + record.other_allowance
    #             - record.pf - record.pt - record.esic - record.employer_pf
    #         )
    ...
    def _compute_monthly_salary(self):
        for record in self:
            ...
            employer_pf = min(math.floor(record.basic_m * 0.12 + 0.5), 1800)
            ...
            record.net_take_home_m = math.floor(
                ...
                + record.other_allowance_m
                - pf
                - record.pt_m  # <-- PT comes from slab compute
                - esic
                - employer_pf
                + 0.5
            )
    ...
    def action_fetch_employee_details(self):
        for record in self:
            ...
            # if contract:
            #     contract.write({
            #         'cost_to_company': record.ctc_m or 0.0,
            ...
            #         'l10n_in_esic_amount': record.basic_m * 0.0075 or 0.0,
            #         'other_allowance': record.other_allowance_m or 0.0,
            #         'work_entry_source': 'attendance',
            #         'date_start': record.date_of_joining,
            #         'employer_pf': record.basic_m * 0.12 or 0.0,
            #         'employer_esic': record.basic_m * 0.0325 or 0.0
            #     })
