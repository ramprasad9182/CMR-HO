...
class order_line_wizard(models.TransientModel):
    ...
    def action_import_barcodes(self):
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            quants = self.env['stock.quant'].search(quant_domain, order='id asc')
            available_quants = quants.filtered(lambda q: q.quantity > 0)
            if not available_quants:
                skipped_lines.append((barcode, "No available stock found for this barcode"))
                continue

            product = available_quants[0].product_id

            # Determine current sale_serial_type
            existing_types = self.env['sale.order.line'].search([
            ...
            if product.tracking == 'serial':
                if qty > len(available_quants):
                    skipped_lines.append(
                        (barcode, f"Requested quantity {qty} exceeds available serials {len(available_quants)}"))
                    continue
                ...
                for q in available_quants:
                    ...
                    # Fetch all order lines using this lot
                    existing_lines = self.env['sale.order.line'].search([
                        ('lot_ids', 'in', [lot.id]),
                        ('order_id.state', 'not in', ['cancel'])
                    ])

                    # 1️⃣ Same order + same serial type
                    same_order_lines = existing_lines.filtered(
                        lambda l: l.order_id.id == self.sale_order_id.id and l.sale_serial_type == sale_serial_type
                    )
                    if same_order_lines:
                        skipped_lines.append(
                            (barcode, f"Serial {lot.name} already used in same order for '{sale_serial_type}' type")
                        )
                        created_count = 0
                        break

                    # 2️⃣ GS1 barcode used in other order → block
                    other_order_lines = existing_lines.filtered(lambda l: l.order_id.id != self.sale_order_id.id)
                    if other_order_lines and 'R' in barcode:
                        other_type = other_order_lines[0].sale_serial_type or 'Undefined'
                        skipped_lines.append(
                            (barcode, f"Serial {lot.name} already used in order {other_order_lines[0].order_id.name} "
                                      f"with type '{other_type}'")
                        )
                        created_count = 0
                        break
                    # 3️⃣ EAN13 → skip if used elsewhere
                    if other_order_lines and 'R' not in barcode:
                        continue
                ...
                if created_count < qty and 'R' not in barcode:
                    skipped_lines.append(
                        (barcode, f"Only {created_count} new serials created out of requested {qty}. "
                                  f"Remaining {qty - created_count} were already used.")
                    )

            # ===================== LOT TRACKED =====================
            else:
                total_available_qty = 0
                for lot in available_quants.mapped('lot_id'):
                    purchase_move_lines = self.env['stock.move.line'].sudo().search([
                        ('company_id.nhcl_company_bool', '=', True),
                        ('lot_id', '=', lot.id),
                        ('location_dest_id', '=', location_id),
                    ])
                    total_available_qty += sum(purchase_move_lines.mapped('qty_done'))

                remaining_qty = qty
                created_any = False

                # Sort lots for predictable split
                for q in available_quants:
                    lot = q.lot_id
                    lot_total = q.quantity
                    ...
                    if same_order_lines:
                        skipped_lines.append(
                            (barcode, f"Lot {lot.name} already used in same order for '{sale_serial_type}' type")
                        )
                        continue

                    other_order_lines = existing_lines.filtered(lambda l: l.order_id.id != self.sale_order_id.id and l.sale_serial_type == sale_serial_type)
                    # if other_order_lines and 'R' in barcode:
                    #     other_type = other_order_lines[0].sale_serial_type or 'Undefined'
                    #     skipped_lines.append(
                    #         (barcode, f"Lot {lot.name} already used in order {other_order_lines[0].order_id.name} "
                    #                   f"with type '{other_type}'")
                    #     )
                    #     continue

                    # Calculate usable qty in this lot
                    lot_used_qty = sum(
                    ...
                    lot_remaining = max(total_available_qty - lot_used_qty, 0)
                    if lot_remaining <= 0:
                        continue

                    # Allocate qty across lots dynamically
                    allocate_qty = min(remaining_qty, lot_remaining)
                    if allocate_qty <= 0:
                        continue
                ...
                if not created_any:
                    skipped_lines.append((barcode, "No valid lot available to allocate quantity"))
                elif remaining_qty > 0:
                    skipped_lines.append(
                        (barcode, f"Only partial quantity allocated. Remaining {remaining_qty} could not be assigned.")
                    )
